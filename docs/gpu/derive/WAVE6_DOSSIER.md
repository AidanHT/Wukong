...
**They live in three modules, not one, and the exception is load-bearing for the ownership question
in section 13.** The log prints each bench with its module: the first two are `serving::tests::*`
(`serving.rs:3101`, `:2468`), the four megakernel rows are `megakernel::tests::mega_vs_*`, and
`decode_stack_latency` is **`gpu::tests::*`** -- the whole-model eager-vs-graph decode-latency bench
at `gpu.rs:25087`. So "the serving benches all live in their own modules" is *nearly* true and
cannot be used as an argument: one of them is in `gpu.rs`, which is the file the campaign puts under
a one-owner-per-wave rule (`CAMPAIGN_CHECKPOINT.md:25-26`).
...
| the owner list includes "the bench wiring in `gpu.rs`" | **KEEP the clause and SCOPE it -- deleting it would strand the round.** Three of this wave's own items are edits inside `gpu.rs` and it cannot run without them: (1) `d1_readbw`'s device-sized reduce grid inside the `hbm_bandwidth` bench (`gpu.rs:23538-23544`) -- the denominator 12.3 refuses to publish without; (2) the ptxas census corpus (`gpu.rs:22250-22335`, test at `:22217`), which G-W6-1 extends to the decode family *before* the GPU is rented; (3) `decode_stack_latency` (`gpu.rs:25087`), the one serving perf bench that does **not** live in its own module (1.3) and the natural host for `d2_launch`. **Amend the clause to name exactly those three touch points** -- "the bench wiring in `gpu.rs`, limited to `hbm_bandwidth`'s reduce arm, the ptxas census corpus and `decode_stack_latency`" -- and everything else the wave writes stays in `paged_attention.rs` / `serving.rs` / `megakernel.rs` / `ptx_norm.rs`, `ptx_wgmma.rs` untouched. The conflict with Wave 3's owner is resolved by the **sequencing** the single-owner rule already implies, not by pretending the clause is unnecessary: the rule is one owner *per wave* and never two agents concurrently (`CAMPAIGN_CHECKPOINT.md:25-26`), and Wave 6 runs after Waves 3-5 in the campaign's own resume order (`CAMPAIGN_CHECKPOINT.md:94-103`), so the two owners are never live at the same time. If the waves are ever overlapped anyway, only item (2) is handable to Wave 3's owner -- (1) and (3) are arms of this round |
