...
- Native (`Obj`/`Exe`) uses Cranelift `emit_object` (no clang, no `.ll`). `Exe` writes a `<stem>_rt.c` runtime and links with `cc` (`$CC` overrides); a missing C compiler returns `UNIMPLEMENTED` (2), a failing link `COMPILE_ERROR` (1). `Obj` needs no external tool.
