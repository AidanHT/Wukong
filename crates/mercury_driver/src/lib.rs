...
fn emit_native(program: &mercury_mir::Program, interner: &Interner, opts: &Options) -> i32 {
    ...
    // links `mercury_runtime` (the AVX2 kernels) as a real dependency, so a recognized-kernel program
    // resolves its `mercury_*` symbols AND the read-only string `.rodata` relocations link. The MinGW
...
enum LinkOutcome {
    ...
    Linked,
    /// rustc or the `mercury_runtime` rlib is not available — the caller should try the `cc` fallback.
    Unavailable,
    /// rustc ran but the link failed (a genuine error — do not silently fall back).
    Failed(String),
}

/// The Rust link shim generated for the rustc-driven `--emit=exe` link. It provides the `mercury_rt_*`
...
/// mercury_runtime` pulls the AVX2 microkernels into the link so a recognized-kernel program resolves
/// its `mercury_*` symbols. `#![no_main]` leaves the Cranelift object's own `main` as the CRT entry.
const MERCURY_RT_SHIM: &str = r##"#![no_main]
//! Auto-generated by `mercuryc --emit=exe`. Do not edit; it is rewritten on every link.
extern crate mercury_runtime;
use std::io::Write;
#[no_mangle]
pub extern "C" fn mercury_rt_print_i64(x: i64) {
    let o = std::io::stdout();
    let _ = write!(o.lock(), "{x}\n");
}
#[no_mangle]
pub extern "C" fn mercury_rt_print_u64(x: u64) {
    let o = std::io::stdout();
    let _ = write!(o.lock(), "{x}\n");
}
#[no_mangle]
pub extern "C" fn mercury_rt_print_f64(x: f64) {
    let o = std::io::stdout();
    let _ = write!(o.lock(), "{x}\n");
}
#[no_mangle]
pub extern "C" fn mercury_rt_print_str(p: *const u8) {
    if p.is_null() {
        return;
    }
    // SAFETY: a Mercury `*u8` string buffer, always NUL-terminated by the string lowering.
    unsafe {
        let mut n = 0isize;
        while *p.offset(n) != 0 {
            n += 1;
...
#[no_mangle]
pub extern "C" fn mercury_rt_assert(c: i64) {
    if c == 0 {
...
#[no_mangle]
pub extern "C" fn mercury_rt_fmod_f64(a: f64, b: f64) -> f64 {
    a % b
}
#[no_mangle]
pub extern "C" fn mercury_rt_fmod_f32(a: f32, b: f32) -> f32 {
    a % b
}
"##;
...
/// automatically) and links `mercury_runtime` as a real dependency, so a recognized-kernel program's
/// `mercury_*` symbols resolve and the string `.rodata` relocations link — neither of which the MinGW
...
fn rustc_link(stem: &str, obj_path: &Path, out: &Path) -> LinkOutcome {
    ...
    let rlib = dir.join("libmercury_runtime.rlib");
    ...
    if let Err(e) = std::fs::write(&shim_path, MERCURY_RT_SHIM) {
    ...
    let status = Command::new("rustc")
        ...
        .arg("2021")
        .arg("--extern")
        .arg(format!("mercury_runtime={}", rlib.display()))
        .arg("-L")
