...
//! instrumented [`mercury_opt::optimize_timed`] pass to attribute time per optimizer pass.
//!
...
//!
//! ```text
//! cargo run -p mercury_bench --release -- compile-time tests/run examples bench/kernels
//! ```
...
pub fn report(files: &[PathBuf]) {
    ...
    let mut agg: Vec<mercury_opt::PassStat> = Vec::new();
...
fn measure_one(path: &Path) -> Meas {
    ...
    let mut a = p0.clone();
    mercury_opt::optimize(&mut a, LEVEL);
    let mut b = p0.clone();
    let _ = mercury_opt::optimize_timed(&mut b, LEVEL);
    if mercury_mir::print::print_program(&a, &interner)
        != mercury_mir::print::print_program(&b, &interner)
    {
    ...
    let opt = best_of(|| {
        let mut c = p0.clone();
        let t0 = Instant::now();
        mercury_opt::optimize(&mut c, LEVEL);
        t0.elapsed()
    });

    // One instrumented run for the per-pass attribution.
    let mut c = p0.clone();
    let timed = mercury_opt::optimize_timed(&mut c, LEVEL);
...
fn lower(src: &str) -> Option<(mercury_mir::Program, Interner)> {
...
fn merge_passes(agg: &mut Vec<mercury_opt::PassStat>, run: &[mercury_opt::PassStat]) {
    for (i, s) in run.iter().enumerate() {
        if agg.len() <= i {
            agg.push(mercury_opt::PassStat {
                name: s.name,
                time: Duration::ZERO,
                calls: 0,
            });
