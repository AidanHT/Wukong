//! `mercury_autodiff` — reverse-mode automatic differentiation as a MIR->MIR transform.
//!
//! Mercury's forward path turns a `.mer` model into tuned kernels; this crate is the **backward
...
//! forward pass runs in `f64` (via `mercury_interp::run_kernel_f64`) so eps-noise stays far below
