...
//!
//! Mercury's GPU path emits PTX and hands it to the driver, which JIT-compiles PTX→SASS on every
//! `cuModuleLoadData`. Within one process the [`crate::gpu::Gpu`] module map already avoids re-JIT,
//! but a fresh process (each `mercuryc --backend=gpu` invocation, each test-binary run) pays the JIT
...
pub fn ptx_to_cubin(ptx: &str) -> Result<Vec<u8>, DriverError> {
    ...
    let name = CString::new("mercury_kernel").unwrap();
    unsafe {
...
/// Directory holding cached cubins. Overridable via `MERCURY_CUBIN_CACHE`; defaults to a subdir of the
/// system temp dir so it survives across process runs but is trivially clearable.
pub fn cache_dir() -> PathBuf {
    std::env::var_os("MERCURY_CUBIN_CACHE")
        .map(PathBuf::from)
        .unwrap_or_else(|| std::env::temp_dir().join("mercury_cubin_cache"))
}
