...
use mercury_mir::{Function, Op, Program, Terminator, ValueId};
...
/// Mirrors `lower::rt_helper` / the `mercury_interp` `Accelerator` recognizers (one entry per
/// `mercury_*` symbol family). A call not in this menu is either a side effect or general code.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CoopKind {
    /// `mercury_sreduce_f32[_parallel](x, y, n, op) -> f32` — dot/ssd/sum/max/min/absmax.
    Reduce,
    /// bf16/f16 reductions: `mercury_{dot,sum,reduce}_{bf16,f16}(...) -> f32`.
    ReduceLowp,
    /// `mercury_sgemm[_parallel]` — `C = A·B (+beta·C)`.
    Gemm,
    /// `mercury_sgemm_nt[_parallel]` — `C = A·Bᵀ`.
    GemmNt,
    /// `mercury_sgemm_nt_epi[_parallel]` — `C = act(A·Bᵀ[+bias])`, the fused FFN/Linear epilogue.
    GemmNtEpi,
    /// `mercury_i8gemm_nt[_parallel]` — int8 (u8×i8→i32) GEMM.
    I8GemmNt,
    /// `mercury_norm_f32[_parallel]` — softmax / LayerNorm / RMSNorm (no affine).
    Norm,
    /// `mercury_norm_affine_f32[_parallel]` — affine LayerNorm/RMSNorm (γ/β).
    NormAffine,
    /// `mercury_vmath_{f32,bf16,f16}(x, out, n, op)` — elementwise activation/transcendental.
    Vmath,
    /// `mercury_vmath2_f32(a, b, out, n, op)` — two-arg pow/atan2/hypot.
    Vmath2,
    /// `mercury_velem_f32[_parallel]` — streaming `act(a·x + b·y + c)` (residual add / fused bias).
    Velem,
    /// `mercury_axpby_{bf16,f16}` — low-precision `y := a·x + b·y`.
    Axpby,
    /// `mercury_parallel_for(n, func_addr F, ctx)` — the generic `@parallel` outliner.
    ParallelFor,
}
...
pub fn classify_call(name: &str) -> CallClass {
    use CoopKind::*;
    let kind = match name {
        "mercury_sreduce_f32" | "mercury_sreduce_f32_parallel" => Reduce,
        "mercury_dot_bf16" | "mercury_dot_f16" | "mercury_sum_bf16" | "mercury_sum_f16"
        | "mercury_reduce_bf16" | "mercury_reduce_f16" => ReduceLowp,
        "mercury_sgemm" | "mercury_sgemm_parallel" => Gemm,
        "mercury_sgemm_nt" | "mercury_sgemm_nt_parallel" => GemmNt,
        "mercury_sgemm_nt_epi" | "mercury_sgemm_nt_epi_parallel" => GemmNtEpi,
        "mercury_i8gemm_nt" | "mercury_i8gemm_nt_parallel" => I8GemmNt,
        "mercury_norm_f32" | "mercury_norm_f32_parallel" => Norm,
        "mercury_norm_affine_f32" | "mercury_norm_affine_f32_parallel" => NormAffine,
        "mercury_vmath_f32" | "mercury_vmath_bf16" | "mercury_vmath_f16" => Vmath,
        "mercury_vmath2_f32" => Vmath2,
        "mercury_velem_f32" | "mercury_velem_f32_parallel" => Velem,
        "mercury_axpby_bf16" | "mercury_axpby_f16" => Axpby,
        "mercury_parallel_for" => ParallelFor,
...
pub struct CoopOp {
    ...
    pub kind: CoopKind,
    /// Resolved symbol name (e.g. `"mercury_sreduce_f32_parallel"`) — distinguishes precision arms.
    pub name: String,
...
fn op_operands(op: &Op) -> Vec<ValueId> {
    match op {
        Op::ConstInt(..) | Op::ConstFloat(..) | Op::Alloca(_) | Op::FuncAddr(_) => Vec::new(),
...
///    `mercury_parallel_for` yet (its outlined body is a separate function — a later increment);
                    ...
                    CallClass::Coop(kind) => {
                        if kind == CoopKind::ParallelFor {
                            return MegaPlan::ineligible(
                                "generic @parallel (mercury_parallel_for) not yet cooperative",
                            );
                        }
...
mod tests {
    ...
    fn build(src: &str, opt: u8) -> Option<(Program, Interner)> {
        ...
        let id = sm.add("fusion_gate.mer".to_string(), src.to_string());
    ...
    #[test]
    fn classify_symbols() {
        assert_eq!(classify_call("mercury_sreduce_f32_parallel"), CallClass::Coop(CoopKind::Reduce));
        assert_eq!(classify_call("mercury_sgemm_nt_epi"), CallClass::Coop(CoopKind::GemmNtEpi));
        assert_eq!(classify_call("mercury_vmath_bf16"), CallClass::Coop(CoopKind::Vmath));
    ...
    /// become `mercury_sreduce_f32_parallel` and `main` inlines them.)
    #[test]
...
"#;
    ...
    /// A recognized small matmul: the loop nest lowers to `mercury_sgemm`; control flow (the GEMM's
    ...
    fn matmul_is_eligible_or_pure_general() {
        let src = std::fs::read_to_string(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tests/run/matmul_f32.mer"),
        )
        .unwrap();
