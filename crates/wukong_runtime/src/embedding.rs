...
/// `out`/`weight`.
#[no_mangle]
pub unsafe extern "C" fn wukong_embedding_f32(
    out: *mut f32,
    weight: *const f32,
    ids: *const i32,
    t: i64,
...
/// **bit-identical to [`wukong_embedding_f32`]** (a permutation/gather, no accumulation — thread
...
mod tests {
    ...
    fn degenerate_shapes_are_noops() {
        ...
        unsafe {
            wukong_embedding_f32(out.as_mut_ptr(), weight.as_ptr(), ids.as_ptr(), 0, 4, 2);
            wukong_embedding_f32(out.as_mut_ptr(), weight.as_ptr(), ids.as_ptr(), 2, 0, 2);
