...
#[no_mangle]
pub unsafe extern "C" fn wukong_lrscan_f32(
    a: *const f32,
    b: *const f32,
    out: *mut f32,
    rows: i64,
...
/// Multicore `out[r,t] = a[r,t]·h_{t-1} + b[r,t]` — **bit-identical** to [`wukong_lrscan_f32`]. Rows are
...
#[no_mangle]
pub unsafe extern "C" fn wukong_lrscan_f32_parallel(
    a: *const f32,
    b: *const f32,
    out: *mut f32,
    rows: i64,
...
mod tests {
    ...
    fn lrscan_known_values() {
        ...
        unsafe {
            wukong_lrscan_f32(a.as_ptr(), b.as_ptr(), got.as_mut_ptr(), 1, 4);
        }
        ...
        unsafe {
            wukong_lrscan_f32(a2.as_ptr(), b2.as_ptr(), got2.as_mut_ptr(), 8, 2);
        }
        assert_eq!(got2, want);
    }
}
