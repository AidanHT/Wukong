...
//! pays. gcc cannot emit that automatically; Wukong's domain-aware lowering *knows* the loop streams a
//! whole tensor once.
//!
...
#[no_mangle]
pub unsafe extern "C" fn wukong_dequant_f32(
    q: *const u8,
    out: *mut f32,
    n: i64,
    scale: f32,
...
/// Multicore dequant — **bit-identical** to [`wukong_dequant_f32`]. The map is elementwise, so `[0,n)`
...
///
/// # Safety
/// Same as [`wukong_dequant_f32`].
#[no_mangle]
pub unsafe extern "C" fn wukong_dequant_f32_parallel(
    q: *const u8,
    out: *mut f32,
    n: i64,
    scale: f32,
    ...
    (0..workers).into_par_iter().for_each(|w| {
        ...
        unsafe {
            ...
            wukong_dequant_f32(q.add(start * in_bytes), out.add(start), len, scale, op);
        }
    });
}
...
#[no_mangle]
pub unsafe extern "C" fn wukong_dequant_perchan_f32(
    q: *const u8,
    out: *mut f32,
    rows: i64,
    cols: i64,
...
/// Multicore per-channel dequant — **bit-identical** to [`wukong_dequant_perchan_f32`] (rows are
...
///
/// # Safety
/// Same as [`wukong_dequant_perchan_f32`].
#[no_mangle]
pub unsafe extern "C" fn wukong_dequant_perchan_f32_parallel(
    q: *const u8,
    out: *mut f32,
    rows: i64,
    cols: i64,
    ...
    fn simd_equals_scalar_twin() {
        ...
        for &act in &[DQ_ID, DQ_RELU, DQ_GELU, DQ_SILU] {
            for &(width, qptr) in &[
                ...
                unsafe {
                    wukong_dequant_f32(qptr, got.as_mut_ptr(), n as i64, scale, op);
                    wukong_dequant_f32_parallel(qptr, got_par.as_mut_ptr(), n as i64, scale, op);
                }
