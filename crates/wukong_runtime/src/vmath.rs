...
// --- Cephes single-precision constants (mirror wukong_mir_build's `exp`/`log` poly constants) ----
...
// --- exp: 8-bucket table reduction (mirrors wukong_mir_build's inlined `emit_exp_f32`) ------------
//
...
// --- log: 8-bucket table reduction (mirrors wukong_mir_build's inlined `emit_log_f32`) ------------
//
...
// --- sin/cos and erf constants (mirror wukong_mir_build's inlined `emit_trig_f32`/`emit_erf_f32`
...
/// consistent with `wukong_vmath_f32` (an unrecognized/sentinel `op` returns `x`, i.e. identity).
#[inline]
...
/// `x` and `out` must each be valid for `n` `f32` elements.
#[no_mangle]
pub unsafe extern "C" fn wukong_vmath_f32(x: *const f32, out: *mut f32, n: i64, op: i64) {
    if n <= 0 {
        return;
    }
    let n = n as usize;
...
/// `6`) keeps the shipped ×4 body; `WUKONG_VMATH_UNROLL=6` selects the ×6 (48-elem) variant so the
...
/// `x`, `y`, and `out` must each be valid for `n` `f32` elements.
#[no_mangle]
pub unsafe extern "C" fn wukong_vmath2_f32(
    x: *const f32,
    y: *const f32,
    out: *mut f32,
    n: i64,
...
/// `wukong_vmath_f32(widen(x), …)` bit-for-bit — the interpreter marshals through this very kernel,
...
#[no_mangle]
pub unsafe extern "C" fn wukong_vmath_bf16(x: *const u16, out: *mut f32, n: i64, op: i64) {
    if n <= 0 {
        return;
    }
    let n = n as usize;
...
#[no_mangle]
pub unsafe extern "C" fn wukong_vmath_f16(x: *const u16, out: *mut f32, n: i64, op: i64) {
    if n <= 0 {
        return;
    }
    let n = n as usize;
...
mod tests {
    ...
    fn vmath_matches_libm() {
        ...
        for &(op, libm, tol) in cases {
            unsafe {
                wukong_vmath_f32(xs.as_ptr(), out.as_mut_ptr(), xs.len() as i64, op);
            }
    ...
    fn vmath_erf_known_values() {
        ...
        let mut out = vec![0.0f32; xs.len()];
        unsafe {
            wukong_vmath_f32(xs.as_ptr(), out.as_mut_ptr(), xs.len() as i64, VM_ERF);
        }
    ...
    fn vmath_inverse_hyperbolic() {
        let check = |op: i64, xs: &[f32], reference: fn(f32) -> f32, tol: f32| {
            let mut out = vec![0.0f32; xs.len()];
            unsafe { wukong_vmath_f32(xs.as_ptr(), out.as_mut_ptr(), xs.len() as i64, op) };
            for (i, &x) in xs.iter().enumerate() {
    ...
    /// The two-input kernel (`wukong_vmath2_f32`): pow/atan2/hypot vs `std`, plus the scalar twin ==
    ...
    fn vmath_tail_matches_lanes() {
        ...
        for op in [
            ...
            unsafe {
                wukong_vmath_f32(xs.as_ptr(), full.as_mut_ptr(), xs.len() as i64, op);
            }
    ...
    fn vmath_log_dense_sweep() {
        ...
        let mut out = vec![0.0f32; xs.len()];
        unsafe {
            wukong_vmath_f32(xs.as_ptr(), out.as_mut_ptr(), xs.len() as i64, VM_LOG);
        }
    ...
    fn vmath_exp_dense_sweep() {
        ...
        let mut out = vec![0.0f32; xs.len()];
        unsafe {
            wukong_vmath_f32(xs.as_ptr(), out.as_mut_ptr(), xs.len() as i64, VM_EXP);
        }
        let mut max_rel = 0.0f64;
        let mut worst = 0.0f32;
        for (i, &x) in xs.iter().enumerate() {
    ...
    fn vmath2_activation_backward() {
        ...
        for (op, gref) in [
            ...
            unsafe {
                wukong_vmath2_f32(xs.as_ptr(), dys.as_ptr(), got.as_mut_ptr(), n as i64, op);
            }
            for i in 0..n {
    ...
    fn vmath2_gate() {
        ...
        for (op, fref) in [
            ...
            unsafe {
                wukong_vmath2_f32(a.as_ptr(), b.as_ptr(), got.as_mut_ptr(), n as i64, op);
            }
            for i in 0..n {
                assert_eq!(
                    got[i].to_bits(),
    ...
    /// bit-for-bit — that equality is exactly what lets the interpreter marshal `wukong_vmath_{bf16,
    ...
    fn vmath_lowp_matches_f32_on_widened() {
        ...
        for op in ops {
            ...
            unsafe {
                wukong_vmath_bf16(bbits.as_ptr(), gb.as_mut_ptr(), n as i64, op);
                wukong_vmath_f32(bwide.as_ptr(), rb.as_mut_ptr(), n as i64, op);
                wukong_vmath_f16(hbits.as_ptr(), gh.as_mut_ptr(), n as i64, op);
                wukong_vmath_f32(hwide.as_ptr(), rh.as_mut_ptr(), n as i64, op);
            }
            for i in 0..n {
    ...
    fn vmath_throughput() {
        ...
        for &(op, name, scalar) in cases {
            ...
            let t_kernel = best(
                iters,
                Box::new(move || unsafe {
                    wukong_vmath_f32(xp as *const f32, outp as *mut f32, n as i64, op);
    ...
    /// Two-input throughput: the 256-bit `wukong_vmath2_f32` kernel vs a scalar `libm`-call loop
    ...
    /// Run: `cargo test -p wukong_runtime --release vmath2_throughput -- --ignored --nocapture`.
    #[test]
    #[ignore = "throughput bench; run explicitly in --release"]
