...
#[no_mangle]
pub unsafe extern "C" fn wukong_cumprod_f32(x: *const f32, out: *mut f32, rows: i64, cols: i64) {
    if rows <= 0 || cols <= 0 {
        return;
    }
...
/// Multicore prefix product — **bit-identical** to [`wukong_cumprod_f32`]. Rows are independent, so the
...
#[no_mangle]
pub unsafe extern "C" fn wukong_cumprod_f32_parallel(
    x: *const f32,
    out: *mut f32,
    rows: i64,
    cols: i64,
...
mod tests {
    ...
    fn cumprod_matches_naive() {
        for &rows in &[1usize, 2, 3, 8, 64, 200] {
            for &cols in &[1usize, 2, 7, 8, 9, 16, 17, 33, 64, 100, 1000] {
                ...
                unsafe {
                    wukong_cumprod_f32(x.as_ptr(), got.as_mut_ptr(), rows as i64, cols as i64);
                }
    ...
    fn serial_equals_parallel_bit_for_bit() {
        ...
        for &cols in &[1usize, 7, 8, 17, 64, 333] {
            ...
            unsafe {
                wukong_cumprod_f32(x.as_ptr(), s.as_mut_ptr(), rows as i64, cols as i64);
                wukong_cumprod_f32_parallel(x.as_ptr(), p.as_mut_ptr(), rows as i64, cols as i64);
            }
    ...
    fn cumprod_known_values() {
        ...
        unsafe {
            wukong_cumprod_f32(x.as_ptr(), got.as_mut_ptr(), 2, 4);
        }
