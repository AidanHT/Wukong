...
//!     row-broadcast stride-0-in-`i` read), so Mercury's affine `velem` recognizer — which requires
...
//!     result is bit-for-bit `mercury_vmath_f32(x + b)`.
//!
...
/// inlined-poly MIR use — so a fused-activation bias is bit-for-bit consistent with `mercury_vmath_f32`.
#[inline]
...
#[no_mangle]
pub unsafe extern "C" fn mercury_bias_bcast_f32(
    x: *const f32,
    b: *const f32,
    out: *mut f32,
    rows: i64,
...
/// Multicore broadcast-bias — the `@parallel` twin of [`mercury_bias_bcast_f32`]. Rows are independent,
...
/// `x` and `out` must each be valid for `rows*cols` `f32`; `b` for `cols` `f32`.
#[no_mangle]
pub unsafe extern "C" fn mercury_bias_bcast_f32_parallel(
    x: *const f32,
    b: *const f32,
    out: *mut f32,
    rows: i64,
    ...
    const RBAND: usize = 8;
    if rows_u <= RBAND {
        return mercury_bias_bcast_f32(x, b, out, rows, cols, op);
    }
    ...
    fn bias_parallel_matches_serial() {
        ...
        for &op in &[BIAS_ACT_NONE, VM_RELU, VM_GELU] {
            ...
            unsafe {
                ...
                mercury_bias_bcast_f32_parallel(
                    x.as_ptr(),
                    b.as_ptr(),
                    par.as_mut_ptr(),
                    rows as i64,
