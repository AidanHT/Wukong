...
/// `dst = srcᵀ` — transpose a `[rows, cols]` row-major matrix into a `[cols, rows]` row-major one,
/// cache-blocked, single-threaded.
///
/// # Safety
/// `src` valid for `rows*cols`, `dst` for `cols*rows` f32, non-overlapping.
#[no_mangle]
pub unsafe extern "C" fn mercury_transpose_f32(src: *const f32, dst: *mut f32, rows: i64, cols: i64) {
    if rows <= 0 || cols <= 0 {
        return;
    }
...
/// Multi-threaded `dst = srcᵀ`: the independent `B`-row blocks are spread across cores. Each block
/// writes a disjoint set of `dst` entries (`dst[*, i]` for `i` in the block), so the result is
/// **bit-identical to the serial transpose** the interpreter marshals as the oracle — there is no
/// accumulation, just a permutation, so thread count and order are irrelevant. Below
/// `TRANSPOSE_PAR_MIN` it runs the serial path.
///
/// # Safety
/// Operand-size contract of [`mercury_transpose_f32`].
#[no_mangle]
pub unsafe extern "C" fn mercury_transpose_f32_parallel(
    src: *const f32,
    dst: *mut f32,
    rows: i64,
    cols: i64,
    ...
    (0..nblocks).into_par_iter().for_each(|blk| {
        ...
        unsafe {
            transpose_rows(src_addr as *const f32, dst_addr as *mut f32, r, c, i0, i1);
        }
    });
}
...
mod tests {
    ...
    fn transpose_matches_naive_and_parallel() {
        for (r, c) in [
            ...
            unsafe {
                mercury_transpose_f32(src.as_ptr(), got.as_mut_ptr(), r as i64, c as i64);
                mercury_transpose_f32_parallel(src.as_ptr(), got_par.as_mut_ptr(), r as i64, c as i64);
            }
