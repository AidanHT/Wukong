...
//! `y·(dy − s)` over the row.
//!
//! The dot is delegated to the **identical bit-exact `mercury_sreduce_f32(RED_DOT)`** the `@parallel`
...
use crate::reduce::{mercury_sreduce_f32, RED_DOT};
...
/// `mercury_sreduce_f32`) then `dx[i] = y[i]·(dy[i] − s)`.
///
/// # Safety
/// `y`, `dy`, `dx` valid for `cols` f32.
#[inline]
unsafe fn softmax_bwd_row(y: *const f32, dy: *const f32, dx: *mut f32, cols: usize) {
    let s = mercury_sreduce_f32(y, dy, cols as i64, RED_DOT);
    #[cfg(target_arch = "x86_64")]
    {
...
#[no_mangle]
pub unsafe extern "C" fn mercury_softmax_bwd_f32(
    y: *const f32,
    dy: *const f32,
    dx: *mut f32,
    rows: i64,
...
/// per-row routine — so the result is **bit-identical to [`mercury_softmax_bwd_f32`]** (rows are
/// independent, no cross-row combine).
///
/// # Safety
/// Operand-size contract of [`mercury_softmax_bwd_f32`].
#[no_mangle]
pub unsafe extern "C" fn mercury_softmax_bwd_f32_parallel(
    y: *const f32,
    dy: *const f32,
    dx: *mut f32,
    rows: i64,
    ...
    if r < SOFTMAX_BWD_PAR_MIN {
        mercury_softmax_bwd_f32(y, dy, dx, rows, cols);
        return;
    }
    use rayon::prelude::*;
    ...
    fn softmax_bwd_matches_reference_and_parallel() {
        for (rows, cols) in [(1, 1), (3, 7), (4, 8), (5, 16), (9, 33), (16, 100)] {
            ...
            for row in 0..rows {
                let off = row * cols;
                let s = unsafe {
                    mercury_sreduce_f32(y[off..].as_ptr(), dy[off..].as_ptr(), cols as i64, RED_DOT)
                };
                for i in 0..cols {
