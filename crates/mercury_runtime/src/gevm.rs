...
//! `[rows, cols]` row-major `A` (`mercury_sgevm_f32[_parallel]`): the single-token KV-decode
...
#[no_mangle]
pub unsafe extern "C" fn mercury_sgevm_f32(
    w: *const f32,
    a: *const f32,
    out: *mut f32,
    rows: i64,
...
/// [`mercury_sgevm_f32`]** (no cross-stripe combine). Below [`GEVM_PAR_MIN`] columns it runs serial.
///
/// # Safety
/// Operand-size contract of [`mercury_sgevm_f32`].
#[no_mangle]
pub unsafe extern "C" fn mercury_sgevm_f32_parallel(
    w: *const f32,
    a: *const f32,
    out: *mut f32,
    rows: i64,
...
mod tests {
    ...
    /// `mercury_*` kernel): the fused fold stays within `√rows·ε` of the exact weighted sum. This is
