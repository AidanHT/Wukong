...
//! [`crate::mercury_rowargmax_i32`] and the value-only column reductions in [`crate::mercury_colmax_f32`].
//!
...
//! (lower) row index.
//!
//! Fast *and* bit-exact via the same row-major, cache-friendly traversal as [`crate::mercury_colsum_f32`]
...
//! `_CMP_GT_OQ` for max, `_CMP_LT_OQ` for min), exactly the lane-update idiom of [`crate::mercury_rowargmax_i32`].
...
/// `x` valid for `rows*cols` `f32`; `out` valid for `cols` `i32`; non-overlapping.
#[no_mangle]
pub unsafe extern "C" fn mercury_colargmax_i32(x: *const f32, out: *mut i32, rows: i64, cols: i64) {
    colarg_serial(x, out, rows, cols, true);
}

/// Multi-threaded `out[j] = argmax_i x[i, j]` (bit-identical to [`mercury_colargmax_i32`]).
///
/// # Safety
/// Operand-size contract of [`mercury_colargmax_i32`].
#[no_mangle]
pub unsafe extern "C" fn mercury_colargmax_i32_parallel(
    x: *const f32,
    out: *mut i32,
    rows: i64,
    cols: i64,
...
/// `x` valid for `rows*cols` `f32`; `out` valid for `cols` `i32`; non-overlapping.
#[no_mangle]
pub unsafe extern "C" fn mercury_colargmin_i32(x: *const f32, out: *mut i32, rows: i64, cols: i64) {
    colarg_serial(x, out, rows, cols, false);
}

/// Multi-threaded `out[j] = argmin_i x[i, j]` (bit-identical to [`mercury_colargmin_i32`]).
///
/// # Safety
/// Operand-size contract of [`mercury_colargmin_i32`].
#[no_mangle]
pub unsafe extern "C" fn mercury_colargmin_i32_parallel(
    x: *const f32,
    out: *mut i32,
    rows: i64,
    cols: i64,
...
mod tests {
        ...
        for (rows, cols) in [
            ...
            for is_max in [true, false] {
                ...
                let (f, fp): (
                    ...
                    (mercury_colargmax_i32, mercury_colargmax_i32_parallel)
                } else {
                    (mercury_colargmin_i32, mercury_colargmin_i32_parallel)
                };
                unsafe {
    ...
    fn duplicate_extrema_return_lowest_row_index() {
        ...
        unsafe {
            mercury_colargmax_i32(x.as_ptr(), amax.as_mut_ptr(), rows as i64, cols as i64);
            mercury_colargmax_i32_parallel(x.as_ptr(), amax_p.as_mut_ptr(), rows as i64, cols as i64);
            mercury_colargmin_i32(x.as_ptr(), amin.as_mut_ptr(), rows as i64, cols as i64);
            mercury_colargmin_i32_parallel(x.as_ptr(), amin_p.as_mut_ptr(), rows as i64, cols as i64);
        }
