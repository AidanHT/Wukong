...
//!
//! **Deterministic parallelism — the hard part, and Mercury's structural advantage.** A C author
...
//! low bits wobble run to run. Mercury instead splits the **output rows**: each rayon chunk owns a
...
#[no_mangle]
pub unsafe extern "C" fn mercury_scatter_add_f32(
    grad_w: *mut f32,
    grad_out: *const f32,
    ids: *const i32,
    t: usize,
...
/// ascending-`ti` folding makes it **bit-identical to [`mercury_scatter_add_f32`]** regardless of thread
...
///
/// # Safety
/// Operand-size contract of [`mercury_scatter_add_f32`].
#[no_mangle]
pub unsafe extern "C" fn mercury_scatter_add_f32_parallel(
    grad_w: *mut f32,
    grad_out: *const f32,
    ids: *const i32,
    t: usize,
...
mod tests {
    ...
    fn scatter_matches_naive_and_parallel() {
        for &(t, h, v) in &[
            ...
            unsafe {
                ...
                mercury_scatter_add_f32_parallel(
                    got_par.as_mut_ptr(),
                    grad_out.as_ptr(),
                    ids.as_ptr(),
                    t,
    ...
    fn heavy_collisions_serial_equals_parallel() {
        ...
        for ids in &id_patterns {
            ...
            let mut got_par = vec![0.0f32; v * h];
            unsafe {
                mercury_scatter_add_f32(got.as_mut_ptr(), grad_out.as_ptr(), ids.as_ptr(), t, h, v);
                mercury_scatter_add_f32_parallel(
                    got_par.as_mut_ptr(),
                    grad_out.as_ptr(),
                    ids.as_ptr(),
    ...
    fn degenerate_shapes_are_noops() {
        ...
        unsafe {
            mercury_scatter_add_f32(grad_w.as_mut_ptr(), grad_out.as_ptr(), ids.as_ptr(), 0, 4, 2);
            mercury_scatter_add_f32(grad_w.as_mut_ptr(), grad_out.as_ptr(), ids.as_ptr(), 2, 0, 2);
            mercury_scatter_add_f32(grad_w.as_mut_ptr(), grad_out.as_ptr(), ids.as_ptr(), 2, 4, 0);
            mercury_scatter_add_f32_parallel(
                grad_w.as_mut_ptr(),
                grad_out.as_ptr(),
                ids.as_ptr(),
