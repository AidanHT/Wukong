...
//! class). It is the per-row sibling of the whole-array [`crate::mercury_argreduce_f32`]: the same
...
/// `x` valid for `rows*cols` `f32`; `out` valid for `rows` `i32`; non-overlapping.
#[no_mangle]
pub unsafe extern "C" fn mercury_rowargmax_i32(x: *const f32, out: *mut i32, rows: i64, cols: i64) {
    rowarg_serial(x, out, rows, cols, true);
}

/// Multi-threaded `out[r] = argmax_j x[r, j]` (bit-identical to [`mercury_rowargmax_i32`]).
///
/// # Safety
/// Operand-size contract of [`mercury_rowargmax_i32`].
#[no_mangle]
pub unsafe extern "C" fn mercury_rowargmax_i32_parallel(
    x: *const f32,
    out: *mut i32,
    rows: i64,
    cols: i64,
...
/// `x` valid for `rows*cols` `f32`; `out` valid for `rows` `i32`; non-overlapping.
#[no_mangle]
pub unsafe extern "C" fn mercury_rowargmin_i32(x: *const f32, out: *mut i32, rows: i64, cols: i64) {
    rowarg_serial(x, out, rows, cols, false);
}

/// Multi-threaded `out[r] = argmin_j x[r, j]` (bit-identical to [`mercury_rowargmin_i32`]).
///
/// # Safety
/// Operand-size contract of [`mercury_rowargmin_i32`].
#[no_mangle]
pub unsafe extern "C" fn mercury_rowargmin_i32_parallel(
    x: *const f32,
    out: *mut i32,
    rows: i64,
    cols: i64,
...
mod tests {
        ...
        for (rows, cols) in [
            ...
            for is_max in [true, false] {
                ...
                let (f, fp): (
                    ...
                    (mercury_rowargmax_i32, mercury_rowargmax_i32_parallel)
                } else {
                    (mercury_rowargmin_i32, mercury_rowargmin_i32_parallel)
                };
                unsafe {
    ...
    fn duplicate_extrema_return_lowest_index() {
        ...
        unsafe {
            mercury_rowargmax_i32(x.as_ptr(), amax.as_mut_ptr(), rows as i64, cols as i64);
            mercury_rowargmax_i32_parallel(x.as_ptr(), amax_p.as_mut_ptr(), rows as i64, cols as i64);
            mercury_rowargmin_i32(x.as_ptr(), amin.as_mut_ptr(), rows as i64, cols as i64);
            mercury_rowargmin_i32_parallel(x.as_ptr(), amin_p.as_mut_ptr(), rows as i64, cols as i64);
        }
    ...
    fn duplicate_extrema_across_lanes_and_parallel_threshold() {
        ...
        unsafe {
            mercury_rowargmax_i32(x.as_ptr(), got.as_mut_ptr(), rows as i64, cols as i64);
            mercury_rowargmax_i32_parallel(x.as_ptr(), got_p.as_mut_ptr(), rows as i64, cols as i64);
        }
