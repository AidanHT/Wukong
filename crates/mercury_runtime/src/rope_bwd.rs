...
#[no_mangle]
pub unsafe extern "C" fn mercury_rope_bwd_f32(
    g: *const f32,
    inv_freq: *const f32,
    dx: *mut f32,
    rows: i64,
...
/// routine — so the result is **bit-identical to [`mercury_rope_bwd_f32`]** (rows are independent, no
/// cross-row combine, so thread count is irrelevant and the interpreter's serial call agrees with this
/// `@parallel` path exactly).
///
/// # Safety
/// Operand-size contract of [`mercury_rope_bwd_f32`].
#[no_mangle]
pub unsafe extern "C" fn mercury_rope_bwd_f32_parallel(
    g: *const f32,
    inv_freq: *const f32,
    dx: *mut f32,
    rows: i64,
    ...
    if r < ROPE_BWD_PAR_MIN {
        mercury_rope_bwd_f32(g, inv_freq, dx, rows, half);
        return;
    }
    let dim = 2 * h;
...
mod tests {
    ...
    fn degenerate_shapes_are_noops() {
        ...
        unsafe {
            mercury_rope_bwd_f32(g.as_ptr(), inv_freq.as_ptr(), dx.as_mut_ptr(), 0, 4);
            mercury_rope_bwd_f32(g.as_ptr(), inv_freq.as_ptr(), dx.as_mut_ptr(), 2, 0);
            mercury_rope_bwd_f32_parallel(g.as_ptr(), inv_freq.as_ptr(), dx.as_mut_ptr(), -1, 4);
        }
