...
#[no_mangle]
pub unsafe extern "C" fn mercury_rope_f32(
    x: *const f32,
    inv_freq: *const f32,
    out: *mut f32,
    rows: i64,
...
/// so the result is **bit-identical to [`mercury_rope_f32`]** (rows are independent, no cross-row
/// combine, so thread count is irrelevant and the interpreter's serial call agrees with this
/// `@parallel` path exactly).
///
/// # Safety
/// Operand-size contract of [`mercury_rope_f32`].
#[no_mangle]
pub unsafe extern "C" fn mercury_rope_f32_parallel(
    x: *const f32,
    inv_freq: *const f32,
    out: *mut f32,
    rows: i64,
    ...
    let (r, h) = (rows as usize, half as usize);
    if r < ROPE_PAR_MIN {
        mercury_rope_f32(x, inv_freq, out, rows, half);
        return;
    }
    let dim = 2 * h;
...
mod tests {
    ...
    fn degenerate_shapes_are_noops() {
        ...
        unsafe {
            mercury_rope_f32(x.as_ptr(), inv_freq.as_ptr(), out.as_mut_ptr(), 0, 4);
            mercury_rope_f32(x.as_ptr(), inv_freq.as_ptr(), out.as_mut_ptr(), 2, 0);
            mercury_rope_f32_parallel(x.as_ptr(), inv_freq.as_ptr(), out.as_mut_ptr(), -1, 4);
        }
