...
#[no_mangle]
pub unsafe extern "C" fn mercury_cumsum_f32(x: *const f32, out: *mut f32, rows: i64, cols: i64) {
    if rows <= 0 || cols <= 0 {
        return;
    }
...
/// Multicore `out[r, i] = Σ_{k=0..=i} x[r, k]` — **bit-identical** to [`mercury_cumsum_f32`]. Rows are
...
/// task).
#[no_mangle]
pub unsafe extern "C" fn mercury_cumsum_f32_parallel(
    x: *const f32,
    out: *mut f32,
    rows: i64,
    cols: i64,
...
mod tests {
    ...
    fn cumsum_matches_scalar_within_tol() {
        for &rows in &[1usize, 3, 64, 500] {
            for &cols in &[1usize, 7, 8, 9, 15, 16, 17, 33, 64, 100, 1000] {
                ...
                unsafe {
                    mercury_cumsum_f32(x.as_ptr(), got.as_mut_ptr(), rows as i64, cols as i64);
                }
    ...
    fn serial_equals_parallel_bit_for_bit() {
        ...
        for &cols in &[1usize, 8, 17, 64, 333] {
            ...
            unsafe {
                mercury_cumsum_f32(x.as_ptr(), s.as_mut_ptr(), rows as i64, cols as i64);
                mercury_cumsum_f32_parallel(x.as_ptr(), p.as_mut_ptr(), rows as i64, cols as i64);
            }
