...
- **Matmul → GEMM dispatch** (runs before generic lowering): `recognize_matmul` matches a matmul loop nest — the `ikj` accumulate form (`match_matmul`) and the textbook `ijk` dot-product form (`match_matmul_ijk`), incl. the `C = A·Bᵀ` (`nn.Linear`) spelling. `match_product_ab` classifies the `A*B` factors (normal `B[k*N+j]` vs transposed `B[j*K+k]`); `MatmulNest` carries `{a,b,c,m,k,n,beta,transposed}`. `matmul_fn` + `lower_matmul_fn` handle a whole-function matmul (incl. `@parallel`); the `lower_for` hook handles an embedded one. `emit_sgemm` emits an `Op::Call` to one of the four `mercury_sgemm*` symbols (bundled in `GemmSyms`, interned in `lower_program`). Dims must be integer literals today.
