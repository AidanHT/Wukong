...
- `src/items.rs` — `impl Parser` for item-level grammar: module header, `fn`/`struct`/`enum`/`const`/`import`/`extern`, generics, params. Public entries `parse_module`/`parse_module_tokens`/`parse_module_tokens_from`.
...
- `parse_module_tokens_from` (`src/items.rs`) — multi-file entry: like `parse_module_tokens` but `NodeId`s start at a caller-supplied base and the next free id is returned. The driver's import loader threads this watermark through each imported file's parse so the merged program's `NodeId`s stay globally unique (sema/mir_build side tables are keyed by them).
...
## Gotchas
- Error recovery, never panics: bad input emits a diag (stable codes E0200–E0209) and synthesizes a placeholder node (e.g. `TupleLit([])` for a missing expr, `«error»` interned ident, `TypeKind::Unit` for a missing type). `module()` guarantees forward progress by bumping if `parse_item` stalls; `recover_item` skips to the next item-starting keyword/`@`/`pub`/Eof.
- `NodeId`s are sequential per-`Parser` (from 0, or from `parse_module_tokens_from`'s base) — unique within one parse run only. Anyone merging modules from several parses MUST thread the returned watermark (as the driver's import loader does), or side tables keyed by `NodeId` silently collide.
