...
- **GEMM marshalling.** On a `mercury_sgemm*` call the interpreter reads the `m*k`/`k*n`/`m*n` f32 operands out of its `Value` memory into real `Vec<f32>` buffers, calls the **identical** `mercury_runtime` kernel the native backend calls (the serial one for both serial/parallel names — numerically identical), then writes the result back. This keeps the differential oracle bit-for-bit exact despite the kernel's reassociated accumulation. (Hence `mercury_runtime` is a runtime dependency now, not just dev.)
