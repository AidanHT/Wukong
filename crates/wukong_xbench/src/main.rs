...
fn bench_matmul_size(cc: &str, dir: &Path, ns: usize, roof: f64) {
    ...
    //      Mer/C(omp) all-core ratios are CONSERVATIVE lower bounds on Wukong — we throttle
...
fn bench_linear(cc: &str, dir: &Path, roof: f64) {
    for ns in [512usize, 1024] {
        ...
        // group (MKL(all), C(omp), Mer(par)) runs last so any residual heat lands on Wukong, never a
...
fn bench_i8gemm(cc: &str, dir: &Path) {
    for ns in [512usize, 1024] {
        ...
        if let Some(m) = &wuk {
            for (lang, other) in [("Mer(par)", &wk_par), ("C", &cm), ("Rust", &rm)] {
                if let Some(o) = other {
                    if o.out != m.out {
                        let at = m
                            .out
...
fn bench_norm(cc: &str, dir: &Path) {
    for &cols in &[768usize, 4096] {
        ...
        println!(
            ...
            "", "Wukong", "C (gcc)", "C(fast)", "Rust", "Mer vs C", "vs C(fast)"
        );
        for op in [
            "softmax",
            "logsoftmax",
