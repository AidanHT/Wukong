...
- `bench_model` (`src/model.rs`) — the **end-to-end model** benchmark: one full CPU inference forward
  ...
  `mercury_interp`). The `@parallel` column is fully multicore (embedded matmul
...
- `bench_norm` (+ `mer_norm`/`c_norm`/`rust_norm`) — fused row normalizations over a `1×cols` feature
  ...
  same basis as the `dot` kernel). All of them copy `x`→`out` then normalize in place (identical work),
...
- **Fairness:** Mercury now contracts `x + y*z` to a fused multiply-add, so gcc gets its *default*
  ...
  Mercury's automatic SIMD+multicore lowering against *idiomatic single-threaded* C/Rust. The C++
  (g++) column runs the identical C body through `g++ -O3 -march=native`; g++ shares gcc's
  middle/back-end, so every disclosure above holds against it exactly as against gcc.
...
- Needs `gcc`, `g++`, and `rustc` on PATH; `CC` / `CXX` override the C / C++ compiler. A missing
  compiler prints a notice and that column shows `n/a` rather than failing the run.
