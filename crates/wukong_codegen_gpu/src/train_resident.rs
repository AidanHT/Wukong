...
mod tests {
    ...
    /// peer for Wukong's training GEMMs (transpose convention is irrelevant for a timing-only peer).
    ...
    /// **M8 honesty frontier** — Wukong's training GEMM vs cuBLAS f32 SGEMM, same-run, at the FFN's
    ...
    fn m8_gemm_vs_cublas() {
        with_gpu("m8_gemm_vs_cublas", |g| {
            for &(m, k, n) in &[(512usize, 768usize, 3072usize), (512, 3072, 768)] {
                ...
                let pct = 100.0 * cub / wuk;
                eprintln!(
                    "GEMM {m}x{n}x{k}: Wukong {:.1} GFLOP/s vs cuBLAS-f32 {:.1} GFLOP/s = {pct:.1}% of cuBLAS",
                    flop / wuk / 1e9,
                    flop / cub / 1e9,
                );
    ...
    /// now that Wukong also uses the tensor cores. Reported, not asserted — it's a measurement.
    #[test]
    #[ignore]
    ...
    // internally, so the eager path needs NO explicit transpose kernel where Wukong's NT-only WMMA
    ...
    /// dtype contract of Wukong's WMMA GEMM): narrows A,B to f16 just-in-time (the price both stacks
    ...
    fn cublas_gemm_rm_narrow(
        ...
        // First cuBLAS operand is Wukong's B, second is Wukong's A; computing column-major Cᵀ[n×m].
        let op_b = if tb {
    ...
    /// One eager-chain resident step on `tr`'s buffers: cuBLAS for the 5 GEMMs, Wukong's own kernels
    ...
    /// **The M8 verdict bench** — Wukong's fused WMMA resident step vs the eager cuBLAS-fp16 chain,
    ...
    /// times both. `fused/eager` > 1 means Wukong wins. Honest expectation: Wukong trails at the
