...
/// `wukong_vmath_f32`. `op` is a `VM_*` code (exp/relu/sigmoid/tanh/silu/gelu so far). Transcendental
...
fn vmath_entry(op: i64) -> &'static str {
    use wukong_runtime::{VM_EXP, VM_GELU, VM_RELU, VM_SIGMOID, VM_SILU, VM_TANH};
    match op {
        x if x == VM_RELU => "relu",
        x if x == VM_EXP => "exp",
...
/// twin of `wukong_sgemm_nt`. Tolerance-gated (the GPU reduces K in a different order).
pub fn gemm_nt(
    g: &mut Gpu,
    a: &[f32],
    b: &[f32],
...
/// `C = A·B` on the GPU: `A` is `m×k`, `B` is `k×n`, `C` is `m×n`. GPU twin of `wukong_sgemm`.
pub fn gemm_nn(
    g: &mut Gpu,
    a: &[f32],
    b: &[f32],
...
/// `WUKONG_FLASH_WS=0` opts out (read once; `OnceLock`, the crate's env idiom) so every dispatch
...
pub(crate) fn ws_flash_enabled() -> bool {
    ...
    *WS.get_or_init(|| !std::env::var("WUKONG_FLASH_WS").is_ok_and(|v| v == "0"))
}
...
/// [`ws_flash_route`] with the env decision injected — the testable core (tests can assert both the
/// enabled and disabled routing tables without mutating process env).
pub(crate) fn ws_flash_route_with(enabled: bool, d: usize, s: usize) -> Option<&'static str> {
    if !enabled {
        return None;
    }
    match d {
...
/// Launch config for the **warp-specialized ping-pong** flash kernels (`flash_d{64,128}_ws*`): 2 warps
/// per CTA, each owning its own 16-query-row tile — grid `ceil((S/16)/2)`, block 64. Multi-head callers
/// set `grid_dim.1 = heads` (the kernel's `ctaid.y` head offset is identical to the `mp` family).
pub(crate) fn ws_flash_cfg(s: usize) -> LaunchConfig {
    LaunchConfig {
        grid_dim: (((s / 16) as u32).div_ceil(2), 1, 1),
        block_dim: (64, 1, 1),
        shared_mem_bytes: 0,
    }
}
...
/// pair when [`ws_flash_enabled`] (default ON; `WUKONG_FLASH_WS=0` opts out) and the shape is in
...
/// **Auto-dispatched** fp16 tensor-core implicit-GEMM conv2d — Wukong's best implicit-GEMM conv path
...
/// **Auto-dispatched** padded/affine conv2d — Wukong's best single-kernel affine-conv path for the
...
/// **Best-path conv2d dispatch** — routes a conv to Wukong's fastest *correct* kernel for the shape:
...
impl ResidentLayerF16 {
    ...
    pub fn new_mha(
        ...
        let f_flash_w = if wmma_flash_applies(dh, s) {
            // Matched (entry, cfg) pair: the env-gated warp-specialized route needs its own 2-warp
            // launch geometry, so the two are resolved together (wmma_flash_plan), never independently.
            let (entry, cfg) = wmma_flash_plan(dh, s, 1);
            let f = g.function("flash", crate::ptx_flash::flash_ptx(), entry)?;
            Some((f, cfg))
        } else {
            None
    ...
    fn run_attn(
        ...
        if self.heads == 1 {
            ...
            // Rebuild the multi-head config from the same plan that picked `f_w` at construction
            // (grid.y = heads); the env-gated ws route changes the block/grid shape, so the config
...
/// **Static-shape-specialized** W4A16 (Wukong's no-library lever, §1A.2): JIT-load a kernel with M/N/K
...
pub fn gemm_nt_int8_smdb(
    ...
    // that is only a 32-multiple falls back to the hand-placed BK=32 kernel. All are bit-exact vs the same
    // i32 oracle, so the choice is purely throughput. Among the swz tiles the perf/gpu-quant-2 sweep found:
    //   • the **w64** tile (128×128 CTA, 4 warps ⇒ 64×64 warp tile) wins once M,N ≥ ~2048 — its 2× per-warp
    //     reuse beats the 64×64 tile there (2048³ ~92%, 4096³ ~84% of cuBLAS, vs the old 8-warp 128 ~79/62%);
    //   • the **64×64** tile wins below that (1024³ ~88% of cuBLAS — more CTAs hide latency when the grid is
...
pub fn gemm_nt_int8_smdb_dequant(
    ...
    let (ptx, entry, bm, bn, warps): (&'static str, &'static str, usize, usize, usize) = if use_w64 {
        ...
        (
            crate::ptx_int8::int8_gemm_smdb_swz_deq_ptx(),
...
mod tests {
    ...
    /// Run: `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture int4_splitk_occupancy`
    #[test]
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu int4_jit_log -- --ignored --nocapture`.
    #[test]
    ...
    fn int4_jit_log() {
        with_gpu("int4_jitlog", |g| {
            ...
            let dump = std::env::temp_dir().join("wukong_w4a16.ptx");
    ...
    fn cpu_vmath(op: i64, x: &[f32]) -> Vec<f32> {
        ...
        unsafe {
            wukong_runtime::wukong_vmath_f32(x.as_ptr(), out.as_mut_ptr(), x.len() as i64, op)
        };
        out
    }
    ...
    fn reductions_match_reference_within_tol_and_are_deterministic() {
        use wukong_runtime::{wukong_sreduce_f32, RED_DOT, RED_SUM};
        with_gpu("reductions", |g| {
            ...
            let cpu_sum = unsafe { wukong_sreduce_f32(x.as_ptr(), x.as_ptr(), n as i64, RED_SUM) };
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu jit_log -- --ignored --nocapture`.
    #[test]
    ...
    /// Wukong's cubin load — the last is fast *and* portable/deterministic (independent of the
    ...
    fn cubin_cache_compile_latency() {
        ...
        with_gpu("cubin_latency", |g| {
            ...
            // Best-of-N: PTX load (driver-JIT-cache warm) vs cubin load (Wukong cache).
...
.visible .entry mma_test(.param .u64 pA, .param .u64 pB, .param .u64 pC)\n{\n\
    ...
    /// Run: `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture`.
    #[test]
    #[ignore = "throughput bench; run explicitly"]
    fn flash_throughput() {
    ...
    /// reduction actually beat Wukong's (already cuDNN-parity) implicit-GEMM on **large feature maps**
    ...
    /// **M6 for conv2d** — Wukong's fp16 tensor-core implicit-GEMM conv (and the f32 SMEM-tiled conv)
    ...
    /// oracle, and both Wukong kernels are checksum-cross-checked against the peer at every shape
    ...
    fn conv_vs_peers() {
        ...
        with_gpu("conv_vs_peers", |g| {
            ...
            for (c, h, wd, k, r, s) in cases {
                ...
                // --- Wukong f32 SMEM-tiled conv (resident; module loaded once) ---
    ...
    /// **The true conv gap (M6, Tier B):** Wukong's fp16 tensor-core implicit-GEMM conv2d vs **cuDNN**
    ...
    /// honest about which cuDNN algorithm we're racing. Correctness gates speed: naive, Wukong, and
    /// cuDNN are each cross-checked against the f64 oracle (fp16 tolerance) and checksum-cross-checked
    /// before any ratio counts. Reports Wukong as **× vs naive** and **% of cuDNN**.
    #[test]
    #[ignore = "throughput bench; needs CUDA + cuDNN redist DLLs on PATH; run explicitly"]
    fn conv_vs_cudnn() {
        use crate::baselines::{
        ...
        with_gpu("conv_vs_cudnn", |g| {
            ...
            for (c, h, wd, k, r, s) in cases {
                ...
                if have_cudnn {
                    eprintln!(
                        "C{c:>3} {h}x{wd} K{k:>3} {r}x{s}: Wukong(sk{sk}) {:>6.0} GF ({:>5.1}× naive, {:>3.0}% cuDNN) | db-pipe {:>4.2}× ({:>6.0} GF) | cuDNN {:>6.0} GF [{}] | naive {:>5.0} GF",
                        g_w / 1e9,
                        g_w / g_n,
                        100.0 * g_w / g_c,
                        g_sb / g_w,
                    ...
                    eprintln!(
                        "C{c:>3} {h}x{wd} K{k:>3} {r}x{s}: Wukong(sk{sk}) {:>6.0} GF ({:>5.1}× naive) | db-pipe {:>4.2}× ({:>6.0} GF) | naive {:>5.0} GF",
                        g_w / 1e9,
                        g_w / g_n,
                        g_sb / g_w,
                        g_sb / 1e9,
    ...
    /// and a 1×1 pointwise. Wukong's [`conv2d_wmma_padded`] vs **cuDNN given the identical pad/stride**
    ...
    fn conv_affine_vs_cudnn() {
        ...
        with_gpu("conv_affine_vs_cudnn", |g| {
            ...
            let have_cudnn = cudnn_available(g);
            if !have_cudnn {
                eprintln!("[warn] cuDNN not loadable — reporting Wukong GFLOP/s only.");
            }
            eprintln!("device: {}", g.device_name());
            ...
            for (c, h, wd, k, r, s, st, pad) in cases {
                ...
                if have_cudnn {
                    eprintln!(
                        "C{c:>3} {h}x{wd} K{k:>3} {r}x{s} s{st}p{pad}: Wukong best {:>3.0}% cuDNN [auto(sk{sk}) {:>5.0} GF / ex {:>5.0} GF] | cuDNN {:>5.0} GF [{}]  (out {p}x{q})",
                        100.0 * g_best / g_c,
                        g_w / 1e9,
                        g_e / 1e9,
                        g_c / 1e9,
                    ...
                    eprintln!(
                        "C{c:>3} {h}x{wd} K{k:>3} {r}x{s} s{st}p{pad}: Wukong auto(sk{sk}) {:>5.0} GF / ex {:>5.0} GF  (out {p}x{q})",
                        g_w / 1e9,
                        g_e / 1e9,
                    );
                }
    ...
    /// **M12 — every GPU kernel is bit-identical run-to-run.** Wukong's kernels use fixed grids and
    ...
    /// **M12 logged win — Wukong is reproducible where cuBLAS makes no such promise.** Asserts
    /// Wukong's fp16 GEMM is bit-identical across three runs (guaranteed by construction), then logs
    ...
    fn reproducibility_vs_cublas() {
        ...
        with_gpu("repro_vs_cublas", |g| {
            ...
            eprintln!("  Wukong: bit-identical = {wk_stable}  (GUARANTEED — fixed grid, no atomics, fixed K order)");
    ...
    /// Wukong, same machine, same buffers, measured same-run (an internal optimized-vs-naive speedup,
    ...
    fn transformer_layer_throughput() {
        with_gpu("transformer_layer_throughput", |g| {
            ...
            for s in [256usize, 512, 1024] {
                ...
                eprintln!(
                    "transformer_layer S={s} D={d} Dff={dff} (same-run, both Wukong):\n  \
    ...
    /// **M13 milestone — Wukong's whole-model resident stack vs the cuBLAS call-chain stack, end-to-end
    /// across depth** (the prompt's "beat the library-call-chain stack end-to-end", the multi-layer
    /// completion of the single-layer `cublas_chain_vs_wukong_layer_throughput`). At each depth N both run
    ...
    /// DLLs on PATH (see `gemm_vs_peers`); skips otherwise. Run: `cargo test -p wukong_codegen_gpu
        ...
        with_gpu("resident_model_vs_cublas", |g| {
            ...
            for depth in [1usize, 2, 4, 8] {
                ...
                let weights: Vec<TransformerWeights> = wdata
                    ...
                    .collect();
                let x = rng.vec(s * d, -1.0, 1.0);
                let wuk = ResidentModelF16::new(g, &weights, s, d, dff).unwrap();
                ...
                let oracle = ref_model_f16(&x, &weights, s, d, dff);
                let wk_out = wuk.forward(&x).unwrap();
                let chain_out = chain.forward(&x).unwrap();
                crate::diff::assert_close(
                    &format!("Wukong model depth={depth}"),
                    &wk_out,
                    &oracle,
                    5e-3,
                    1e-1,
                ...
                let t_mer = best_of(ROUNDS, || {
                    let t0 = Instant::now();
                    for _ in 0..iters {
                        let _ = wuk.forward_device(&x_d).unwrap();
                    }
                    g.stream.synchronize().unwrap();
                    t0.elapsed().as_secs_f64() / iters as f64
                });
                ...
                eprintln!(
                    "M13 stack depth={depth} S={s} D={d} Dff={dff} (resident, same-run): \
                     Wukong {:.3} ms ({:.3} ms/layer, {:.0} tok/s) | cuBLAS chain {:.3} ms ({:.3} ms/layer) \
                     || Wukong {:.2}× faster end-to-end",
                    t_mer * 1e3,
                    t_mer * 1e3 / dl,
                    s as f64 / t_mer,
                    t_chain * 1e3,
    ...
    /// fused Wukong layer uses; (c) a cross-check that the cuBLAS chain and Wukong's fused WMMA layer
    ...
    /// **M13 milestone bench — Wukong's fused fp16 stack vs the cuBLAS GEMM call-chain** (the literal
    /// "library call-chain" the milestone must beat), Tier B, same machine, same buffers, same-run. Three
    /// resident timings per S: Wukong *fused* ([`ResidentLayerF16::forward_device`] — WMMA with the two
    /// residual adds & SiLU folded into the GEMMs), Wukong *unfused*
    ...
    /// cast glue). The decomposition is the point: **Wukong-unfused vs cuBLAS** isolates GEMM quality (same
    ...
    /// DLLs on PATH (see `gemm_vs_peers`); skips otherwise. Run: `cargo test -p wukong_codegen_gpu
    /// --features gpu --release -- --ignored --nocapture cublas_chain_vs_wukong`.
    #[test]
    #[ignore = "needs CUDA redist DLLs on PATH; throughput bench; run explicitly"]
    fn cublas_chain_vs_wukong_layer_throughput() {
        ...
        with_gpu("cublas_chain_vs_wukong", |g| {
            ...
            // feed WUKONG_MS_PER_LAYER in bench/pytorch/transformer_layer_peer.py.
            for s in [256usize, 512, 1024, 2048, 4096] {
                let x = rng.vec(s * d, -1.0, 1.0);
                let wq = rng.vec(d * d, -0.1, 0.1);
                let wk = rng.vec(d * d, -0.1, 0.1);
                ...
                let t_unf = best_of(ROUNDS, || {
                    let t0 = Instant::now();
                    for _ in 0..iters {
                        let _ = wuk.forward_device_unfused(&x_d).unwrap();
                    }
                    g.stream.synchronize().unwrap();
                    t0.elapsed().as_secs_f64() / iters as f64
                });
                ...
                let (hdl_lbl, hdl) = if t_chain >= t_fused {
                    ("Wukong fused faster", t_chain / t_fused)
                } else {
                    ("cuBLAS chain faster", t_fused / t_chain)
                };
                eprintln!(
                    "M13 layer S={s} D={d} Dff={dff} (resident, same-run): \
                     Wukong fused {:.3} ms ({:.0} tok/s) | Wukong unfused {:.3} ms | cuBLAS chain {:.3} ms \
                     || {hdl_lbl} {hdl:.2}× | GEMM-quality (Wukong-unfused vs cuBLAS) {:.2}× | fusion {:.2}×",
                    t_fused * 1e3,
                    s as f64 / t_fused,
                    t_unf * 1e3,
                    t_chain * 1e3,
    ...
    /// `cublas_chain_vs_wukong_layer_throughput` runs a D=64 toy; this is a genuine transformer layer.
    /// Wukong's fused multi-head [`ResidentLayerF16`] vs the **multi-head** cuBLAS-chain layer
    ...
    /// skips if absent. Run:
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture cublas_chain_vs_wukong_mha`.
    #[test]
    #[ignore = "throughput bench; needs CUDA redist DLLs on PATH; run explicitly"]
    fn cublas_chain_vs_wukong_mha_layer_throughput() {
        use crate::baselines::{peer_env_hint, peers_available, CublasChainLayer};
    ...
    /// Run with `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture`.
    #[test]
    #[ignore = "throughput bench; run explicitly"]
    fn gemm_throughput() {
    ...
    /// speed number, the peer must agree with the *same* E4M3-rounded f64 reference Wukong's own fp8
    ...
    /// Wukong's fp8 (E4M3) GEMM as a **% of cuBLASLt fp8** — the real M2 peer (cuBLASLt is the *only*
    /// cuBLAS surface with an fp8 matmul). Same-run, same E4M3 bytes fed to both, so the % is
    /// clock-invariant (absolutes swing ~7× with Ada's boost). Reports both Wukong kernels: the
    ...
    /// different instant than Wukong), but timing the *two Wukong kernels back-to-back, interleaved
    ...
    /// **The honest three-tier GPU scoreboard for fp16 GEMM** — the Phase-0 payoff. Wukong's
    /// tensor-core GEMM measured *same-run, same buffers* against the only peers that mean anything on
    /// a GPU: NVIDIA's hand-tuned **cuBLAS** (Tier B, the gold standard — Wukong is reported as a % of
    /// it) and a **naive CUDA-C** kernel compiled by NVRTC (Tier A — the literal "beat the C a
    /// programmer writes," the GPU twin of beating scalar CPU-C). No Wukong-GPU-vs-CPU-C comparison
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture gemm_vs_peers`
    ...
    fn gemm_vs_peers() {
        ...
        with_gpu("gemm_vs_peers", |g| {
            ...
            // --- Speed: same-run fp16 GEMM, Wukong vs cuBLAS (Tier B) vs naive CUDA-C (Tier A). ---
            ...
            for sz in [1024usize, 2048, 4096] {
                ...
                // Wukong fp16 WMMA fragment-reuse path (same kernel as tensorcore_roofline_pct).
                let a = rng.vec(m * k, -1.0, 1.0);
                let b = rng.vec(n * k, -1.0, 1.0);
                let a16: Vec<f16> = a.iter().map(|&x| f16::from_f32(x)).collect();
                let b16: Vec<f16> = b.iter().map(|&x| f16::from_f32(x)).collect();
                ...
                eprintln!(
                    "\n{sz}³ fp16 GEMM (same-run):\n  \
                     Wukong _mt      : {:>7.0} GFLOP/s  | {:>5.1}% of cuBLAS\n  \
                     Wukong _sm        : {:>7.0} GFLOP/s  | {:>5.1}% of cuBLAS | {:>5.2}× vs _mt\n  \
                     Wukong _sm_db     : {:>7.0} GFLOP/s  | {:>5.1}% of cuBLAS | {:>5.2}× vs _sm | {:>5.1}× vs naive\n  \
                     Wukong _sm128     : {:>7.0} GFLOP/s  | {:>5.1}% of cuBLAS | {:>5.2}× vs _sm\n  \
                     Wukong _sm128db   : {:>7.0} GFLOP/s  | {:>5.1}% of cuBLAS | {:>5.2}× vs _sm\n  \
    ...
    /// Run: `cargo test -p wukong_codegen_gpu --features gpu -- --ignored --nocapture nvrtc_wmma_probe`.
    #[test]
...
"#;
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture gemm_pipe_sweep`.
    #[test]
    #[ignore = "throughput bench; needs CUDA redist DLLs on PATH; run explicitly"]
    fn gemm_pipe_sweep() {
        use crate::baselines::{
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture gemm_cliff_ab`.
    #[test]
    #[ignore = "throughput bench; needs CUDA redist DLLs on PATH; run explicitly"]
    fn gemm_cliff_ab() {
        use crate::baselines::{
        ...
        with_gpu("gemm_cliff_ab", |g| {
            ...
            for sz in [2048usize, 4096] {
                ...
                // (`gemm_ex_nt_f16_f32out`) — the **apples-to-apples** peer for Wukong's f32-C kernels,
                // since the f16-out peer writes C at half the width and so hides ~half the epilogue
                // traffic Wukong pays. Both peer columns print per variant.
                ...
                eprintln!(
                    "  cuBLAS peers: f16-out {:>7.0} GFLOP/s | f32-out {:>7.0} GFLOP/s (f32-out is the apples-to-apples peer for Wukong's f32-C kernels; the {:.2}× is the C-write dtype asymmetry)",
                    flop / best_cub / 1e9,
    ...
    /// Needs standalone `ptxas` (`pip install nvidia-cuda-nvcc-cu12`): set `WUKONG_PTXAS`, else the nvcc
    ...
    fn gemm_cliff_ptxas_ab() {
        ...
        with_gpu("gemm_cliff_ptxas", |g| {
            ...
            let ptx_path = dir.join(format!("wukong_cliff_{pid}.ptx"));
    ...
    /// unfused attention chain**, same-run, single head, D=64. Wukong's `flash_d64_mp` (tensor-core
    ...
    /// *library* bar. Wukong's fused flash never spills the S×S scores, so the cuBLAS-chain gap **is**
    /// the value of fusion and grows with S. Reports GFLOP/s (`4·S²·D`) and Wukong × vs each at
    ...
    /// absent. Run:
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture flash_vs_peers`.
    #[test]
    #[ignore = "throughput bench; needs CUDA redist DLLs on PATH; run explicitly"]
    fn flash_vs_peers() {
        use crate::baselines::{
        ...
        with_gpu("flash_vs_peers", |g| {
            ...
            // --- Correctness first: naive (f32 in) and Wukong flash (f16 in) both match the f64 oracle. ---
            ...
            for s in [512usize, 1024, 2048, 4096] {
                ...
                eprintln!(
                    "S={s:>4} D={d} (1 head, same-run): Wukong flash {:.4} ms ({:>6.0} GF/s) | cuBLAS chain {:.4} ms ({:>6.0} GF/s) | naive {:.4} ms ({:>5.0} GF/s) || Wukong {:>4.1}× vs cuBLAS-chain, {:>5.1}× vs naive",
                    t_m * 1e3,
                    g_m / 1e9,
                    t_c * 1e3,
                    g_c / 1e9,
            ...
            //     starves at small S. [H,S,D] layout; Wukong launches grid (S/16, H, 1). ---
            let heads = 12usize;
            for s in [512usize, 1024, 2048] {
                ...
                eprintln!(
                    "H={heads} S={s:>4} D={d} (same-run): Wukong flash {:.4} ms ({:>6.0} GFLOP/s) | naive CUDA-C {:.4} ms ({:>5.0} GFLOP/s) || Wukong {:>5.1}× vs naive",
                    t_m * 1e3,
                    g_m / 1e9,
                    t_n * 1e3,
                    g_n / 1e9,
        ...
        with_gpu("fused_act", |g| {
            ...
            for sz in [512usize, 1024, 2048] {
                ...
                for act in ["relu", "silu", "gelu"] {
                    ...
                    eprintln!(
                        "  {act}: fused {:>7.3} ms ({:>6.0} GFLOP/s) | Wukong chain {:>7.3} ms ({:>4.2}× slower) | cuBLAS chain {:>7.3} ms (fused {:>4.2}× faster)",
                        t_fused * 1e3, flop / t_fused / 1e9,
                        wk_chain * 1e3, wk_chain / t_fused,
                ...
                eprintln!(
                    "  residual: fused {:>7.3} ms ({:>6.0} GFLOP/s) | Wukong chain {:>7.3} ms ({:>4.2}× slower) | cuBLAS chain {:>7.3} ms (fused {:>4.2}× faster)",
                    t_fused_r * 1e3, flop / t_fused_r / 1e9,
                    wk_chain_r * 1e3, wk_chain_r / t_fused_r,
    ...
    /// **The honest W4A16 (int4 weight-only decode) scoreboard — M4 / M6.** Wukong's int4-decode GEMM
    /// measured *same-run, same buffers* against (Tier A) a **naive CUDA-C W4A16** kernel compiled by
    /// NVRTC — the literal "beat the hand-written int4 decode kernel" — and against Wukong's **own
    ...
    ///
    /// Correctness gates speed (first law): Wukong and the naive peer are first cross-checked against
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture int4_gemm_vs_peers`
    #[test]
    #[ignore = "throughput bench; needs CUDA redist DLLs on PATH; run explicitly"]
    fn int4_gemm_vs_peers() {
        ...
        with_gpu("int4_gemm_vs_peers", |g| {
            ...
            // --- Correctness first: Wukong W4A16 + the naive peer both match the f64 dequant oracle. ---
            ...
            for (m, k, n) in [
                ...
                // Wukong W4A16 — resident packed int4 weights + fp16 activations.
                let a16: Vec<f16> = a.iter().map(|&x| f16::from_f32(x)).collect();
                let a_d = g.stream.memcpy_stod(&a16).unwrap();
                let bq_d = g.stream.memcpy_stod(&qw.packed).unwrap();
                let scl_d = g.stream.memcpy_stod(&qw.scales).unwrap();
                ...
                // Static-shape-specialized kernel (M/N/K baked) — Wukong's no-library lever. ptxas
                ...
                eprintln!(
                    "\n{m}x{k}x{n} W4A16 (same-run):\n  \
                     Wukong W4A16     : {:>8.0} GFLOP/s | {:>6.1}× vs naive CUDA-C | {:>5.2}× vs Wukong fp16 (same tile)\n  \
                     Wukong W4A16 stat: {:>8.0} GFLOP/s | {:>5.2}× vs dynamic (same raw-JIT load; M/N/K baked)\n  \
                     Wukong fp16      : {:>8.0} GFLOP/s | full fp16 weights — the HBM traffic int4 avoids\n  \
    ...
    /// two times are ~equal → Wukong gets the `i32→f32·scale[j]` dequant at ≈0 marginal cost. A cuBLAS
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture int8_dequant_fusion`
    #[test]
    #[ignore = "throughput bench; run explicitly"]
    ...
    /// `s8` reinterpretations coincide, so Wukong's `u8×s8`, the NVRTC peers, and cuBLAS's `s8×s8` all
    /// compute the *identical* matrix and cross-check bit-for-bit — see the cuBLAS signedness caveat in
    /// `baselines.rs`). Returns A both as `u8` (Wukong/naive/dp4a) and as `i8` (cuBLAS), plus i8 B.
    fn int8_inputs_a127(
    ...
    /// **M3/M6: int8 (W8A8) tensor-core GEMM vs the Tier-A int8 CUDA-C peers**, same-run. Wukong's
    ...
    /// nails the Tier-A wins and the dp4a bar first). Reports int8-MAC GFLOP/s (`2·M·N·K`) and Wukong
    /// × vs each peer.
    ///
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture int8_gemm_vs_peers`
    #[test]
    #[ignore = "throughput bench; needs CUDA redist DLLs on PATH; run explicitly"]
    fn int8_gemm_vs_peers() {
        ...
        with_gpu("int8_gemm_vs_peers", |g| {
            ...
            // shape. Activations in [0,127] (see int8_inputs_a127) so cuBLAS's s8×s8 == Wukong's u8×s8.
            ...
            for sz in [1024usize, 2048, 4096] {
                ...
                // Wukong fragment-reuse _mt path (the fast int8 kernel).
                ...
                eprintln!(
                    "\n{sz}³ int8 W8A8 GEMM (same-run, 2·M·N·K MAC-FLOP):\n  \
                     Wukong _smdb128 : {:>8.0} GFLOP/s  | {:>6.1}% of cuBLAS\n  \
                     Wukong _smdb64  : {:>8.0} GFLOP/s  | {:>6.1}% of cuBLAS\n  \
                     Wukong _smdb*   : {:>8.0} GFLOP/s  | {:>6.1}% of cuBLAS | {:>6.1}× vs naive | {:>5.2}× vs dp4a | {:>5.2}× vs _mt\n  \
                     Wukong _mt      : {:>8.0} GFLOP/s  | {:>6.1}% of cuBLAS | {:>6.1}× vs naive | {:>5.2}× vs dp4a\n  \
                     Wukong single   : {:>8.0} GFLOP/s  | {:>6.1}% of cuBLAS | {:>6.1}× vs naive | {:>5.2}× vs dp4a\n  \
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture int8_smdb_sweep`
    #[test]
    #[ignore = "throughput bench; needs CUDA redist DLLs on PATH; run explicitly"]
    fn int8_smdb_sweep() {
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture quant_int8_raster_sweep`
    #[test]
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture quant_int8_bigtile_sweep`
    #[test]
    #[ignore = "throughput sweep; needs CUDA redist DLLs on PATH; run explicitly"]
    fn quant_int8_bigtile_sweep() {
        ...
        with_gpu("quant_int8_bigtile_sweep", |g| {
            ...
            const ROUNDS: usize = 6;
            // (label, bm, bn, wm, wn, raster). Isolates two levers: (a) WARP tile at constant CTA tile —
            // 128×128 w4×2 (shipped, 32×64 warp tile) vs w2×2 (64×64 warp tile, half the warps); (b) CTA
            // tile — 256×128 / 128×256 (64×64 warp tile, 8 warps), ±threadblock rasterization.
            let variants: [(&str, usize, usize, usize, usize, usize); 8] = [
                ("128x128 w4x2    ", 128, 128, 4, 2, 0), // shipped baseline (32×64 warp tile)
                ("128x128 w2x2    ", 128, 128, 2, 2, 0), // 64×64 warp tile, 4 warps — warp-tile lever
    ...
    /// **int8 w64 (64×64-warp-tile) finalization sweep — settles the default per size (perf/gpu-quant-2).**
    /// The big-tile sweep found the 64×64 warp tile (128×128 CTA, 4 warps) is the lever; this compares the
    /// three shipping candidates — the 64×64-CTA swz tile (the prior small-size default), the new w64 (128×128
    /// CTA / 64×64 warp tile), and w64+raster=8 — vs cuBLAS, same-run interleaved, at 1024³/2048³/4096³. Lean
    /// (3 variants) so it can be re-run ≥3× for stability (the binding law). **Bit-exact** checksum first. Run:
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture quant_int8_w64_confirm`
    #[test]
    #[ignore = "throughput sweep; needs CUDA redist DLLs on PATH; run explicitly"]
    fn quant_int8_w64_confirm() {
        ...
        with_gpu("quant_int8_w64_confirm", |g| {
            ...
            eprintln!("device: {}", g.device_name());
            let w_small = INT8_WARPS_M * INT8_WARPS_N;
            ...
            for sz in [1024usize, 2048, 4096] {
                ...
                let b_d = g.stream.memcpy_stod(&b).unwrap();
                let mut c_d = g.stream.memcpy_stod(&vec![0i32; m * n]).unwrap();
                eprintln!("\n{sz}³ int8 w64-finalization vs cuBLAS (same-run, interleaved):");
                let mut best: Option<(String, f64)> = None;
                for (label, ptx, entry, bm, bn, warps, raster) in variants {
                    if m % bm != 0 || n % bn != 0 {
                        eprintln!("  {label}: n/a");
                        continue;
                    }
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture quant_int8_w64_s3_sweep`
    #[test]
    #[ignore = "throughput sweep; needs CUDA redist DLLs on PATH; run explicitly"]
    ...
    /// within a tight tolerance. Several 128-tiling shapes. Skips without a GPU.
    #[test]
    fn quant_int8_w64_dequant_matches_reference() {
        use crate::ptx_int8::{
            int8_gemm_w64_swz_deq_ptx, INT8_W64_BM, INT8_W64_BN, INT8_W64_WARPS_M, INT8_W64_WARPS_N,
        };
        with_gpu("quant_int8_w64_deq", |g| {
            let warps = INT8_W64_WARPS_M * INT8_W64_WARPS_N;
    ...
    /// **int8 fused GEMM+dequant vs the cuBLAS GEMM+dequant chain — the cuBLAS-can't-fuse headline
    /// (perf/gpu-quant-2).** A quantized-inference output stage is `out = f32(Σ u8·i8)·scale[j]`. cuBLAS
    /// int8 emits raw `i32`, so it needs a **second** kernel that re-reads the whole M×N i32 from HBM and
    /// writes M×N f32 — Wukong folds that dequant into the GEMM store for ~0 cost (`int8_gemm_nt_*_deq`).
    /// Now that the int8 GEMM is near cuBLAS parity (w64), the fusion makes Wukong's single kernel **beat
    /// the cuBLAS GEMM+dequant chain outright** — the lever a closed library can't use. Times Wukong's
    /// dispatched fused kernel vs the cuBLAS (GEMM + dequant) chain and cuBLAS GEMM-only, same-run
    /// interleaved, at 1024³/2048³/4096³. Needs the redist DLLs on PATH. Run:
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture quant_int8_fused_dequant_vs_chain`
    #[test]
    #[ignore = "throughput bench; needs CUDA redist DLLs on PATH; run explicitly"]
    fn quant_int8_fused_dequant_vs_chain() {
        use crate::baselines::{
        ...
        with_gpu("quant_int8_fused_dequant_vs_chain", |g| {
            ...
            for sz in [1024usize, 2048, 4096] {
                ...
                let f = g.function(entry, ptx, entry).unwrap();
                let cfg = int8_smdb_cfg(m, n, bm, bn, warps);
                let a_d = g.stream.memcpy_stod(&vec![1u8; m * k]).unwrap();
                ...
                eprintln!(
                    "\n{sz}³ int8 output stage = act-free Linear out (same-run):\n  \
                     Wukong FUSED gemm+dequant : {:>8.0} GFLOP/s\n  \
                     cuBLAS GEMM + dequant chain: {:>8.0} GFLOP/s\n  \
                     cuBLAS GEMM only (no dequant): {:>8.0} GFLOP/s\n  \
                     → Wukong fused BEATS the cuBLAS chain {:>5.2}× (the chain pays {:+.1}% over its own GEMM for the\n    \
                       separate i32→f32 HBM round-trip; Wukong folds it into the GEMM store for ~0)",
                    flop / s_fused / 1e9,
                    flop / s_chain / 1e9,
                    flop / s_gemm / 1e9,
                    s_chain / s_fused,
                    100.0 * (s_chain - s_gemm) / s_gemm,
                );
            }
        });
    }

    /// **fp8 warp-tile cross-check gate (first law).** The int8 win was the 64×64 **warp** tile (4 warps);
    /// fp8 shares the same `mma.sync.m16n8k32` 8-bit geometry, so a 128×128 CTA with wm=wn=2 (a 64×64 warp
    /// tile) is the transferable lever. A change of warp tiling only repartitions which warp computes which
    /// output element — the per-element f32 K-accumulation order (k=0,32,64,…) is unchanged — so the wm=2,wn=2
    /// config must produce the **same** fp8 result as the shipped wm=2,wn=4 default. Checks checksum equality
    /// at a small shape. Skips without a GPU.
    #[test]
    fn quant_fp8_warp_tile_matches() {
        use crate::ptx_fp8::{f32_to_e4m3, fp8_pipe_cfg_ptx};
    ...
    /// **fp8 warp-tile %-of-cuBLASLt sweep — the transferable int8 lever (perf/gpu-quant-2).** The shipped
    /// fp8 pipe runs a 128×128 CTA with wm=2,wn=4 (a 64×32 warp tile); the int8 sweep found the **64×64**
    /// warp tile (wm=wn=2, 4 warps) wins by doubling per-warp reuse. fp8 is the same `mma.sync.m16n8k32`
    /// 8-bit geometry, so this sweeps the warp tiling (64×32 default, **64×64**, 32×64, +a 3-stage 64×64) vs
    /// cuBLASLt fp8 E4M3, same-run, at 1024³/2048³/4096³. Cross-checks each config's checksum == the default
    /// (same K-accumulation order) before timing. Needs the redist DLLs on PATH. Run:
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture quant_fp8_warp_tile_sweep`
    #[test]
    #[ignore = "throughput sweep; needs cublasLt64_12.dll on PATH; run explicitly"]
    fn quant_fp8_warp_tile_sweep() {
        ...
        with_gpu("quant_fp8_warp_tile_sweep", |g| {
            ...
            let configs: [(&'static str, usize, usize, usize, usize, usize, usize, usize); 4] = [
                ...
                ("fp8_w22_64x64_s3", 128, 128, 32, 2, 2, 3, 16),
            ];
            let mut rng = crate::diff::Rng::new(0xF8E0);
            let wa = rng.vec(2048 * 2048, -1.0, 1.0);
            let wb = rng.vec(2048 * 2048, -1.0, 1.0);
            for _ in 0..15 {
                let _ = gemm_nt_fp8_pipe(g, &wa, &wb, 2048, 2048, 2048).unwrap();
    ...
    /// under one clock state and reports the **Wukong-internal** swz/handplaced ratio — which stays
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture int8_swz_vs_handplaced`
    #[test]
    #[ignore = "throughput bench; run explicitly"]
    ...
    /// large-K shapes, same-run; the ratio is Wukong-internal (no cuBLAS) so it stays honest under
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release -- --ignored --nocapture int8_splitk_occupancy`
    #[test]
    #[ignore = "throughput bench; run explicitly"]
    ...
    /// **The real bar: Wukong's fused flash vs a *genuinely fused* FA2-class peer.** [`flash_vs_peers`]
    /// only beats the *unfused* cuBLAS chain (the pre-FlashAttention baseline) — but a real fused FA2
    /// kernel beats that chain too, so that comparison never proved Wukong's standing vs the SOTA. This
    /// bench drives **cuDNN's fused attention** (and cutlass mem-efficient fMHA) through PyTorch SDPA
    /// ([`crate::baselines::fa2_sdpa_peer`]) over the IDENTICAL f16 Q/K/V bytes Wukong's `flash_d64_mp`
    /// runs. Each output is tolerance-gated against the per-head f64 oracle (small S) and checksum-cross-
    /// checked against Wukong (all S). Wukong is wall-clock timed (Rust launch overhead <1%); the peer
    /// is CUDA-event timed (its Python dispatch overhead excluded) — so any Wukong win is the
    /// *conservative* direction. `REPS` back-to-back repetitions on a warmed clock; the unfused MATH
    /// backend is reported as a cross-anchor to Wukong's existing chain ratio.
    #[test]
    #[ignore = "fused-peer bench; needs the torch-CUDA venv (set WUKONG_FA2_PYTHON) + a GPU"]
    fn attn_vs_fused_peer() {
        ...
        with_gpu("attn_vs_fused_peer", |g| {
            ...
            for &s in &[512usize, 1024, 2048, 4096] {
                ...
                // Wukong output once for the cross-check.
                launch_m(g, &mut o_d);
                g.stream.synchronize().unwrap();
                let out_m = g.stream.memcpy_dtov(&o_d).unwrap();
                ...
                if s <= 512 {
                    ...
                    let sm = crate::diff::assert_close(
                        &format!("Wukong flash H={heads} S={s}"),
                        &out_m,
                        &oracle,
                        3e-3,
                        3e-2,
                    ...
                    eprintln!(
                        "[gate] S={s}: Wukong max_abs={:.2e} | {} peer max_abs={:.2e} (both vs f64 oracle) ✓",
                ...
                assert!(
                    ...
                    "H={heads} S={s}: Wukong vs {} peer checksum disagree: wuk={cs_m:.4e} peer={cs_p:.4e}",
                    rep.chosen
                );

                // Speed — REPS back-to-back (the peer re-warms each rep). Best Wukong time vs the peer's
                ...
                eprintln!(
                    "H={heads} S={s:>4} D={d}: Wukong {:.4} ms ({:>6.0} GF/s) | {} fused {:.4} ms ({:>6.0} GF/s) || Wukong {:.2}× {} || cuDNN {:>6.0} | efficient {:>6.0} | MATH(unfused) {:>5.0} GF/s",
                    best_m * 1e3,
                    g_m / 1e9,
                    peer_name,
                    best_peer_sec * 1e3,
    ...
    /// Sweep Wukong flash **kernel variants** against the same cuDNN fused peer, same-run — the
    ...
    #[test]
    #[ignore = "fused-peer variant sweep; needs WUKONG_FA2_PYTHON + a GPU"]
            ...
            for &s in &[512usize, 1024, 2048, 4096] {
                ...
                for ((f, w), (label, _, _)) in funcs.iter().zip(variants.iter()) {
                    ...
                    assert!(
                        ...
                        "S={s} {label}: checksum vs peer wuk={cs:.3e} peer={cs_p:.3e}"
                    );
                    let t = best_of(ROUNDS, || {
                        let t0 = Instant::now();
                        for _ in 0..50 {
    ...
    /// [`attn_causal_vs_fused_peer`]: identical f16 Q/K/V to Wukong and the PyTorch-SDPA fused peer,
    /// output gated vs the per-head f64 oracle at small S, checksum-cross-checked, Wukong wall-clock vs
    /// the peer's CUDA-event time (so Wukong wins are conservative), REPS=3 on a warmed clock. Covers the
        ...
        with_gpu("attn_lm_vs_fused_peer", |g| {
            ...
            for &(entry, d, causal) in &configs {
                ...
                for &s in &[512usize, 1024, 2048, 4096] {
                    ...
                    assert!(
                        (cs_m - cs_p).abs() / cs_p.max(1.0) < 3e-2,
                        "{entry} S={s}: Wukong vs {} checksum disagree wuk={cs_m:.4e} peer={cs_p:.4e}",
                        rep.chosen
                    );
                    let flop = attn_flop(heads, s, d);
                    let mut best_m = f64::INFINITY;
                    ...
                    let gf = |o: Option<f64>| o.map_or(f64::NAN, |sec| flop / sec / 1e9);
                    eprintln!(
                        "{entry} H={heads} S={s:>4} D={d}: Wukong {:.4} ms ({:>6.0} GF/s) | {} fused {:.4} ms ({:>6.0} GF/s) || Wukong {:.2}× {} || cuDNN {:>6.0} | efficient {:>6.0} GF/s",
    ...
    /// **The fused-RoPE win the library can't touch** — Wukong's `flash_d64_mprope` (one kernel that
    ...
    /// while Wukong folds the rotation into the b32 mma fragments it already loads, ~free on a kernel
    /// that's tensor-core-bound. We report Wukong vs **both** the peer's full rope+sdpa pipeline (what a
    /// model actually pays) and its sdpa-only time (so the RoPE tax the library forces is visible).
    ///
    /// Honesty mirrors [`attn_vs_fused_peer`]: same f16 Q/K/V + cos/sin to both; Wukong's fused-rope O is
    /// checksum-cross-checked against the peer's rope+sdpa O (and at small S both gated vs an f64 oracle
    /// that rotates then attends); Wukong wall-clock vs the peer's CUDA-event time (peer's launch
    /// overhead excluded ⇒ Wukong wins are conservative); REPS back-to-back on a warmed clock. Small/
    /// moderate S, where Wukong's flash is near parity and the RoPE round-trip is the largest fraction.
    #[test]
    #[ignore = "fused-RoPE peer bench; needs the torch-CUDA venv (set WUKONG_FA2_PYTHON) + a GPU"]
    fn attn_rope_vs_fused_peer() {
        ...
        with_gpu("attn_rope_vs_fused_peer", |g| {
            ...
            for &s in &[256usize, 512, 1024, 2048] {
                ...
                // Wukong fused-rope output once for the cross-check.
                launch_m(g, &mut o_d);
                g.stream.synchronize().unwrap();
                let out_m = g.stream.memcpy_dtov(&o_d).unwrap();
                ...
                // per head; at all S checksum-cross-check Wukong vs the peer's rope+sdpa O.
                if s <= 512 {
                    let rope_apply = |x: &[f32]| -> Vec<f32> {
                        let mut o = x.to_vec();
                        for p in 0..s {
                    ...
                    let sm = crate::diff::assert_close(
                        &format!("Wukong rope-flash H={heads} S={s}"),
                        &out_m,
                        &oracle,
                        3e-3,
                        3e-2,
                    ...
                    eprintln!(
                        "[gate] S={s}: Wukong rope-flash max_abs={:.2e} | {} rope+peer max_abs={:.2e} (both vs f64 oracle) ✓",
                        sm.max_abs, rep.chosen, sp.max_abs
                    );
                }
                let csum = |v: &[f32]| v.iter().map(|x| x.abs() as f64).sum::<f64>();
                ...
                assert!(
                    (cs_m - cs_p).abs() / cs_p.max(1.0) < 3e-2,
                    "H={heads} S={s}: Wukong rope-flash vs {} rope+peer checksum disagree: wuk={cs_m:.4e} peer={cs_p:.4e}",
                    rep.chosen
                );

                // Speed — REPS back-to-back. Wukong's ONE kernel vs the peer's rope+sdpa pipeline; we also
                ...
                eprintln!(
                    "H={heads} S={s:>4} D={d}: Wukong(fused-rope) {:.4} ms ({:>6.0} GF/s) | {} rope({rope_kind})+sdpa {:.4} ms ({:>6.0} GF/s) [sdpa-only {:.4} ms ({:>6.0} GF/s), RoPE tax {:.4} ms = {:>4.0}%] || Wukong {:.2}× the pipeline, {:.2}× sdpa-only",
                    best_m * 1e3,
                    g_m,
                    peer_name,
                    best_pipe * 1e3,
    ...
    /// **Causal flash vs cuDNN/cutlass causal SDPA** (`is_causal=true`). Wukong's `flash_d64_mpc` skips
    ...
    /// identical f16 Q/K/V. The honest question: does the clean triangular skip let Wukong's causal
    /// ratio *beat* its non-causal ~0.4–0.8× (i.e. is Wukong's skip more efficient than cuDNN's)? Output
    /// is gated vs the per-head f64 [`ref_attn_causal`] oracle at small S and checksum-cross-checked at
    /// all S. Same honesty model as the non-causal bench (Wukong wall-clock vs peer CUDA-event time;
    ...
    #[test]
    #[ignore = "fused-peer causal bench; needs the torch-CUDA venv (set WUKONG_FA2_PYTHON) + a GPU"]
        ...
        with_gpu("attn_causal_vs_fused_peer", |g| {
            ...
            for &s in &[512usize, 1024, 2048, 4096] {
                ...
                if s <= 512 {
                    ...
                    let sm = crate::diff::assert_close(
                        &format!("Wukong causal flash H={heads} S={s}"),
                        &out_m,
                        &oracle,
                        3e-3,
                        3e-2,
                    ...
                    eprintln!(
                        "[gate] S={s}: Wukong causal max_abs={:.2e} | {} causal peer max_abs={:.2e} (both vs f64 oracle) ✓",
                        sm.max_abs, rep.chosen, sp.max_abs
                    );
                }
                let csum = |v: &[f32]| v.iter().map(|x| x.abs() as f64).sum::<f64>();
                ...
                assert!(
                    (cs_m - cs_p).abs() / cs_p.max(1.0) < 3e-2,
                    "H={heads} S={s}: Wukong causal vs {} causal peer checksum disagree: wuk={cs_m:.4e} peer={cs_p:.4e}",
                    rep.chosen
                );
                ...
                let gf = |o: Option<f64>| o.map_or(f64::NAN, |sec| flop / sec / 1e9);
                eprintln!(
                    "H={heads} S={s:>4} D={d} CAUSAL: Wukong {:.4} ms ({:>6.0} GF/s) | {} fused {:.4} ms ({:>6.0} GF/s) || Wukong {:.2}× {} || cuDNN {:>6.0} | efficient {:>6.0} GF/s  [GF/s = full-S² conv.]",
                    best_m * 1e3,
                    g_m / 1e9,
                    peer_name,
                    best_peer_sec * 1e3,
    ...
    /// **Correctness gate (law #1) for the warp-specialized ping-pong flash** (`flash_d64_ws{c}` /
    /// `flash_d128_ws{c}_lm`) and its env-gated dispatch seam. The kernel splits the K-loop across two
    /// anti-phased warp instruction streams coupled by named barriers — a wrong barrier phase, a stage
    /// issued before the other warp's V-read retires, or a botched ragged/causal predicate scatters O or
    /// deadlocks (a divergence deadlock surfaces as a launch error, not a hang). Per-row math is identical
    /// to `flash_d{d}_mp`, so it must match the f64 oracle at the same tolerance. Shapes: S=16 and S=48
    /// exercise the ragged single-active-warp CTA (odd S/16 → warp B inactive: it must still stage and hit
    /// every barrier), S=32/64 the 1-2 CTA edges, S=256/512 the steady pipeline, S=2048 the dispatch
    /// regime; causal additionally exercises warp A idling through warp B's diagonal block. Also checksum-
    ...
    fn ws_flash_matches_reference_within_tol() {
        ...
        with_gpu("ws_flash", |g| {
            ...
            let to16 = |x: &[f32]| -> Vec<f16> { x.iter().map(|&v| f16::from_f32(v)).collect() };
            let back = |x: &[f16]| -> Vec<f32> { x.iter().map(|&v| v.to_f32()).collect() };
            for &(d, entry_nc, entry_c) in &[
                (64usize, "flash_d64_ws", "flash_d64_wsc"),
                (128, "flash_d128_ws_lm", "flash_d128_wsc_lm"),
            ] {
                ...
                for &s in &[16usize, 32, 48, 64, 256, 512, 2048] {
                    ...
                    for (entry, causal) in [(entry_nc, false), (entry_c, true)] {
                        ...
                        let got = g.stream.memcpy_dtov(&o_d).unwrap();
                        let oracle = if causal {
                            ref_attn_causal(&qf, &kf, &vf, s, d, scale)
                        } else {
                            ref_attn(&qf, &kf, &vf, s, d, scale)
                        };
                        let st = crate::diff::assert_close(
                            &format!("{entry} s={s}"),
                            &got,
            ...
            // Checksum cross-check vs the production kernels at the dispatch regime (multi-head grid.y,
            // S=2048, H=4): the ws kernels must reproduce mp / mp_lm within the f32-order tolerance.
            let heads = 4usize;
            let s = 2048usize;
            for &(d, ws_e, mp_e) in &[
                (64usize, "flash_d64_ws", "flash_d64_mp"),
                (128, "flash_d128_ws_lm", "flash_d128_mp_lm"),
            ] {
                let n = heads * s * d;
                let q16 = g.stream.memcpy_stod(&to16(&rng.vec(n, -1.0, 1.0))).unwrap();
                let k16 = g.stream.memcpy_stod(&to16(&rng.vec(n, -1.0, 1.0))).unwrap();
                ...
                assert!(
                    ...
                    "d={d} S={s} H={heads}: ws vs mp checksum disagree mp={:.3e} ws={:.3e}",
                    sums[0],
                    sums[1]
                );
    ...
    /// `WUKONG_FLASH_WS=1` becomes the default long-S dispatch (and whether the deeper ring beats the
    ...
    /// `cargo test -p wukong_codegen_gpu --features gpu --release flash_ws_vs_mp -- --ignored --nocapture`.
    #[test]
    #[ignore = "tuning A/B; run explicitly"]
    fn flash_ws_vs_mp() {
        use half::f16;
        with_gpu("flash_ws_vs_mp", |g| {
            let mut rng = crate::diff::Rng::new(0x75FA5B);
            let to16 = |x: &[f32]| -> Vec<f16> { x.iter().map(|&v| f16::from_f32(v)).collect() };
            let wa = rng.vec(1024 * 1024, -1.0, 1.0);
            let wb = rng.vec(1024 * 1024, -1.0, 1.0);
            let pin = |g: &mut Gpu| {
            ...
            for &(label, base_e, ws_e, ws3_e, d) in &triples {
                ...
                for &s in &[512usize, 1024, 2048, 4096] {
                    ...
                    let scale = 1.0f32 / (d as f32).sqrt();
                    let ss = s as u32;
                    let mut cfg_base = wmma_flash_cfg(s);
    ...
    /// Wukong's `flash_d128_mp` and the fused peer, output gated vs the per-head f64 oracle at small S
    /// and checksum-cross-checked, Wukong wall-clock vs the peer's CUDA-event time, REPS on a warmed
    /// clock. Establishes Wukong's D=128 standing against the genuinely fused FA-class peers.
    #[test]
    #[ignore = "fused-peer D=128 bench; needs the torch-CUDA venv (set WUKONG_FA2_PYTHON) + a GPU"]
    fn attn_d128_vs_fused_peer() {
        ...
        with_gpu("attn_d128_vs_fused_peer", |g| {
            ...
            for &s in &[512usize, 1024, 2048, 4096] {
                ...
                let gf = |o: Option<f64>| o.map_or(f64::NAN, |sec| flop / sec / 1e9);
                eprintln!(
                    "H={heads} S={s:>4} D=128: Wukong {:.4} ms ({:>6.0} GF/s) | {} fused {:.4} ms ({:>6.0} GF/s) || Wukong {:.2}× {} || cuDNN {:>6.0} | efficient {:>6.0} GF/s",
                    best_m * 1e3, g_m / 1e9, peer_name, best_peer_sec * 1e3, g_p / 1e9, g_m / g_p, peer_name, gf(cud), gf(eff),
                );
            }
        });
