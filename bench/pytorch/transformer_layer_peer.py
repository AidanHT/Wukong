#!/usr/bin/env python
"""Tier-C GPU peer: PyTorch pre-norm transformer layer, same GPU as Wukong.

The M13 milestone is "beat the library-call-chain stack end-to-end AND beat PyTorch". Wukong's
fused fp16 ResidentLayerF16 / ResidentModelF16 already beat the cuBLAS call-chain (Tier B, measured
in-process by `cublas_chain_vs_wukong_layer_throughput` / `resident_model_vs_cublas_chain_throughput`).
This script is the **PyTorch** bar (Tier C): the *same* mathematical layer, built the way a PyTorch
user would, timed on the *same* RTX 4050 so the numbers are comparable to Wukong's Rust bench.
...
The two laws hold here too:
  ...
  * HONESTY — same machine, same shapes (D=64, Dff=256, S in {256,512,1024}); clock warmed + best-of-N
    ...
    fp16 flash SDPA), i.e. it is represented at its best, not handicapped to Wukong's exact dtype
    boundaries. This is a cross-language, cross-process comparison (torch in Python vs Wukong in
    Rust) — like `wukong_xbench` for CPU C/Rust — NOT a same-process same-buffer measurement; both
    sides are warmed/best-of-N/synchronized and compute the same function to the same tolerance.

Wukong's measured ms/layer on this GPU (from the Rust bench, fp16 fused resident, same shapes) are
printed alongside for convenience — they are the authoritative Wukong figures; re-run the Rust bench
to refresh them.
...
# Wukong fp16 fused resident, measured same-GPU (cublas_chain_vs_wukong / resident_model_vs_cublas),
...
# run-to-run far beyond best-of-N's reach (the *same* Wukong layer measured 0.23 and 0.385 ms on two
# runs — a ~1.7× clock swing), so the torch/Wukong *ratio* is order-of-magnitude only, not precise. The
# whole set below is ONE consistent run (so the Wukong column is at least self-consistent across S); the
# rigorous long-context claim is the same-process flash A/B + cuBLAS same-run bench, not this ratio.
WUKONG_MS_PER_LAYER = {256: 0.633, 512: 0.813, 1024: 0.745, 2048: 1.151, 4096: 1.250}  # one run, REGISTER-RESIDENT mma.sync flash (flash_d64_m)
WUKONG_STACK_MS_PER_LAYER = {1: 0.34, 2: 0.34, 4: 0.34, 8: 0.34}  # depth sweep, S=512 (WMMA flash)
# Wukong isolated single-head flash_d64_mp GFLOP/s — the cp.async-pipelined mma.sync flash, at PEAK
...
# Wukong timings). For the M5 %-of-(torch SDPA / FA2) column. Cross-process ⇒ clock-approximate (~7×
...
WUKONG_FLASH_GFLOPS = {512: 3371.0, 1024: 5726.0, 2048: 6794.0, 4096: 7003.0}


def rmsnorm_f32(x):
    """RMSNorm with no learned scale, eps=1e-5, computed in f32 (matches Wukong's f32 norm kernel)."""
    xf = x.float()
...
class TorchLayer:
    ...
    def __init__(self, gen, dtype):
        self.dtype = dtype
        # Weights [out, in], row-major — F.linear / `x @ W.t()` computes Wukong's x.W^T (no bias).
    ...
    def forward(self, x):
        ...
        f1 = h2 @ self.W1.t()
        f1a = F.silu(f1.float()).to(self.dtype)  # SiLU in f32 (matches Wukong's vmath silu)
        f2 = f1a @ self.W2.t()
        return x1 + f2.float()                   # residual 2 (f32)


def ref_forward_f64(layer, x):
    """f64 reference of the identical function — the correctness oracle (cf. Wukong's ref_*)."""
    f64 = torch.float64
...
def main():
    ...
    # Wukong's flash is f32; torch's fp16 SDPA can use the fp16 backends. Allow them all (fast path).
    try:
    ...
    # Wukong's flash_d64_mp is single-head D=64; torch SDPA on [1,1,S,D] dispatches its fused flash /
    # mem-efficient kernel (production FA2-class). Same shape, same fp16 in. GFLOP/s = 4·S²·D. The
    # Wukong column is the Rust flash_pipe_vs_mma peak number (same GPU); the ratio is %-of-(torch SDPA),
    ...
    for S in (512, 1024, 2048, 4096):
        ...
        wuk = WUKONG_FLASH_GFLOPS.get(S)
        tail = f"| Wukong flash {wuk:.0f} GFLOP/s | Wukong {wuk / gflops * 100:.0f}% of torch SDPA" if wuk else ""
    ...
    # the GPU at small S. Wukong multi-head GFLOP/s come from the Rust flash_vs_peers multi-head sweep.
    ...
    # 2048/4096 = long context: attention (O(S²·D), torch's flash SDPA vs Wukong's WMMA flash)
    # dominates the FFN, so these sizes test the flash kernels head-to-head — the regime where torch's
    # production fused SDPA is strongest and Wukong's hand-rolled WMMA flash is most exposed.
    for S in (256, 512, 1024, 2048, 4096):
        ...
        wuk = WUKONG_MS_PER_LAYER[S]
        print(
            f"  S={S}: torch eager fp16 {ms:.3f} ms/layer | Wukong fused {wuk:.3f} ms/layer "
            f"| Wukong {ms/wuk:.2f}x faster  (torch max_abs={a_e:.2e})"
        )
    ...
    for S in (256, 512, 1024):
        ...
        try:
            ...
            wuk = WUKONG_MS_PER_LAYER[S]
            print(
                f"  S={S}: torch.compile fp16 {ms:.3f} ms/layer | Wukong fused {wuk:.3f} ms/layer "
                f"| Wukong {ms/wuk:.2f}x faster  (torch max_abs={a_e:.2e})"
            )
    ...
    for depth in (1, 2, 4, 8):
        ...
        wuk = WUKONG_STACK_MS_PER_LAYER[depth]
        print(
            f"  depth={depth}: torch eager fp16 {ms/depth:.3f} ms/layer ({ms:.3f} ms total) "
            f"| Wukong {wuk:.3f} ms/layer | Wukong {(ms/depth)/wuk:.2f}x faster"
        )
