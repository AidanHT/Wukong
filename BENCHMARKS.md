...
| CPU vmath | oneMKL VML | **CLOSED 2026-08-06** — the residual was never algorithmic, it was the **dispatch call boundary**: the elementwise loop reached its 8-lane kernel through a *function pointer*, and because this crate is built without crate-wide AVX a `__m256` cannot cross a real call in a register, so every 8 lanes paid a 32-byte argument spill, a `vzeroupper`, an indirect `call`, a 32-byte reload — and the poly constants could not stay in ymm across the loop. (Verified in the emitted asm, not inferred.) Monomorphizing the op dispatch (one `match` outside the loop, kernel inlined) is **bit-identical on all 2³² f32** and is worth **1.12–1.75×** on its own — the power-independent internal A/B, `WUKONG_VMATH_FNPTR=1` restoring the old spelling in the same binary. Same-run vs VML, n = 2¹²…2²³, best-of-30/40 ABBA, 1 thread, **three rounds across both power states** (battery 32–35%, AC 14%, AC 28%): log **1.17–1.46× FASTER** at every size and tanh **4.08–4.66×** — both unambiguous. exp is the honest one: **1.08–1.25× faster at 2¹² and 2²⁰, a TIE at 2¹⁶** (readings 1.008× slower … 1.053× faster over four runs — parity, not a win), and **1.97–2.13× at 2²³**. What is unambiguous at every exp size is the *delta*: the fnptr arm in the same process reads 1.20–1.49× **slower** than VML at n ≤ 2²⁰, reproducing the old row this one replaces, so the change moved exp from a clear loss to parity-or-better. (fnptr log 1.05–1.28× slower, fnptr tanh 2.56–3.20×.) The two-input kernel (`wukong_vmath2_f32`: activation backward, SwiGLU/GeGLU gates, pow/atan2/hypot), which spilled **two** `__m256` per call, took the identical fix for **1.09–1.48×** (no library peer exists for `silu'`/`gelu'`, so only the internal A/B is quoted). Accuracy re-swept **exhaustively**: exp 1.625e-7 / ≤2 ULP over all 2.24e9 f32 in [−87, 88.376], log 6.924e-7 / ≤12 ULP over all 2.13e9 positive normals | the four *arithmetic* levers were all tried and none of them was the answer — 8-bucket in-register LUTs (which did close the former ~1.7–2× loss), Estrin scheduling, the ×4/×6 ILP unroll, and a bit-identical ldexp exp tail that measured a ~10–15% LOSS and was reverted. Caveats kept: only the mono-vs-fnptr ratio is power-independent — the vs-VML ratios are same-run but still power-sensitive, which is why both states are quoted; and the ≫L3 end of the exp range (≈2×) is partly Wukong's non-temporal store regime, not core throughput. The bf16/f16-input dispatchers still use the function-pointer table (bandwidth-bound, and 36 more monomorphizations each is a code-size trade that was not measured). **Unresolved instrument disagreement, pre-dating this change**: `wukong-xbench exp` has its *own* VML peer, and it prints a much larger ratio than the row above — 2.59× (new) / 1.59× (old, via `WUKONG_VMATH_FNPTR=1`) at N = 2²⁰ where the in-crate instrument reads ≈1.2× / ≈0.8×. Both agree on the **delta** this change makes (xbench 361.1 → 254.1 µs = 1.42×, in-crate 1.39–1.75×), so the standing above does not rest on the difference; but xbench times its arms in separate tight rep-blocks while the in-crate one rotates four arms over ~20 MiB of buffers, so the two are measuring different cache regimes and xbench's row has been contradicting this table's *direction* since before 2026-08-06. **The pessimistic instrument is the one quoted here.** Not investigated further |
