...
- **rustc-driven link path and the generated `#![no_main]` Rust runtime shim** — `crates/wukong_driver/src/lib.rs:639-686` — Locates the `wukong_runtime` rlib from the compiler exe's directory via `runtime_rlib_in` (the uplifted `libwukong_runtime.rlib`, else a hash-suffixed `deps/libwukong_runtime-*.rlib` — cargo uplifts only for a `build`) and links through rustc so kernel symbols and `.rodata` relocations resolve; shim at `crates/wukong_driver/src/lib.rs:578-630`.
